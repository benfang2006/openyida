...
完整应用先由 \`yida-requirement-analysis\` 生成共享需求简报，再并行执行两个独立 artifact：\`yida-prd\` 写 \`prd/<项目名>/prd.md\`，负责业务目标、数据结构、页面与功能、资源顺序、导航顺序和验收标准；\`yida-design\` 写 \`prd/<项目名>/design.md\`，负责主题色、themeProfile、tokens、视觉系统、组件和状态规则。\`yida-app\` 必须等待两份产物完成并通过 join 后才能创建业务资源。页面实现先读 PRD 的页面场景、页面区块、数据来源、主操作和表单入口，再读 design.md 的主题、布局、材质、组件和状态规则，然后交给 \`yida-canvas-custom-page\` 落地。只有已识别为历史平台 JSX 组件页面维护时，才由 \`yida-custom-page\` 自身闭环处理。
