...
所有模式的 PRD 都必须写入视觉规范（主题 profile、主题色来源、明暗模式、导航视觉、页面气质/布局）；fast_build 只写轻量视觉规范，不升级成长 PRD。完整应用主页面生成默认包含 \`yida-page-uiux\` 轻量 ui_skill 引导，产出页面类型、模板路由、\`visualProfile\` 和去 sample 化检查，再交给 \`yida-canvas-custom-page\` 或 \`yida-custom-page\` 落地。这不等于 \`deep_design\`。
