...
const SKILL_COVERAGE = {
  ...
  'yida-requirement-analysis': { level: 'offline-unit', tests: ['skill metadata and packaging validation', 'tests/skill-contracts.test.js'], reason: 'writes only a local structured requirement brief used by the parallel PRD and visual artifact owners' },
  'yida-prd': { level: 'offline-unit', tests: ['skill metadata and packaging validation', 'routing eval scenarios', 'tests/skill-contracts.test.js'], reason: 'writes only the local Product PRD artifact and is validated through routing, ownership and join contracts without mutating Yida resources' },
  'yida-design': { level: 'offline-unit', tests: ['skill metadata and packaging validation', 'routing eval scenarios', 'tests/create-form.test.js', 'tests/skill-contracts.test.js'], reason: 'writes only the visual design artifact from the shared requirement brief; validate routing, packaging, visual ownership and form theme payloads rather than mutating Yida resources or tenant-wide theme config' },
  ...
  'yida-canvas-custom-page': { level: 'offline-unit', tests: ['skill metadata and packaging validation'], reason: 'YidaCodeCanvas authoring is published through the shared publish command; shared real E2E validates metadata and routing here, while page mutation remains covered by dedicated publish scenarios' },
  ...
  'yida-canvas-upgrade': { level: 'offline-unit', tests: ['skill metadata and packaging validation'], reason: 'platform JSX component page migration emits source/report artifacts and may publish through .canvas.jsx after verification; shared real E2E validates metadata and routing to avoid overwriting existing pages' },
