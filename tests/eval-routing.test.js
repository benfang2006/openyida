...
describe('eval routing', () => {
  ...
  test('evaluateScenario 校验默认建访客系统走 yida-app 统一编排且包含产品设计', () => {
    const scenario = {
      id: 'visitor-default-build',
  ...
  test('evaluateScenario 允许统一编排默认加载 yida-design 和 Canvas，但加载数据源判为未命中', () => {
    const fakeAgent = () => ({
      available: true,
      json: {
        skill: 'yida-app',
    ...
    expect(r.forbiddenDefaultSkillHits).toEqual(['yida-data-source-connectors']);
  });
