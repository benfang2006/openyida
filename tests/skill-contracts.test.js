...
describe('OpenYida skill contracts', () => {
  ...
  test('custom pages do not build page-level navigation by default', () => {
    ...
    expect(pageUiux).toContain('先分清两类隐藏');
    expect(pageUiux).toContain("应用导航隐藏写 `appBlueprint.hideAppNav: 'y'`");
    ...
    expect(navStep).toContain('先区分两件事');
    expect(navStep).toContain('自定义页要做顶部导航、侧边导航、导航壳、自绘应用级导航');
    expect(navStep).toContain("写 `appBlueprint.hideAppNav: 'y'`，并交给 `yida-nav-shell`");
    ...
    expect(navPatterns).toContain('不用 `isRenderNav=false` 表达应用导航隐藏');
    expect(navShell).toContain('openyida update-app <appType> --hide-app-nav');
    expect(navShell).toContain('自定义页最终 URL 不依赖 `isRenderNav=false` 隐藏应用导航');
    expect(navShell).toContain('先记住三句话');
