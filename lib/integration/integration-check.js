...
function appendFlow(result, group, flow) {
  ...
  result.flows.push({
    ...
    formName: group.formName || group.title || flow.formName || '',
...
async function listAllFlowsForForm(authRef, appType, formUuid, baseGroup, options = {}) {
  ...
  do {
    if (pageIndex > maxPages) {
      const error = new Error('Integration form flow list exceeded the safe pagination limit.');
      error.code = 'INTEGRATION_LIST_PAGINATION_LIMIT';
      throw error;
    }
  ...
  return flows.flows.map((flow) => ({
    ...
    formName: flow.formName || baseGroup.formName || baseGroup.title || '',
...
async function listAllLogicflows(authRef, appType, options = {}) {
  ...
  for (const type of flowTypes) {
    ...
    do {
      if (pageIndex > maxPages) {
        const error = new Error('Integration flow list exceeded the safe pagination limit.');
        error.code = 'INTEGRATION_LIST_PAGINATION_LIMIT';
        throw error;
      }
    ...
    for (const group of groupsWithMore) {
      const formFlows = await listAllFlowsForForm(authRef, appType, group.formUuid, group, {
        pageSize,
        type,
        maxPages,
        key: options.key || '',
