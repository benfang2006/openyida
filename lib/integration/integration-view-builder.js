...
function buildViewJson(options) {
  const {
    ...
    dataFormUuid, dataFormType, dataFormName, dataSourceFormName,
  ...
  if (hasDataNode && dataNodeId) {
    ...
    const dataSourceFormType = resolveDataSourceFormType(
      ...
      { defaultFormType: dataFormType, defaultFormName: resolvedDataFormName, formEventTypes },
    );
    ...
    const conditions = (viewDataConditions || dataConditions) && (viewDataConditions || dataConditions).length > 0
      ? buildDataRetrieveCondition(viewDataConditions || dataConditions)
