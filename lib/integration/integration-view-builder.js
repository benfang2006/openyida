...
function buildViewJson(options) {
  const {
    ...
    dataFormUuid, dataFormType, dataFormName, dataSourceFormName, dataSourceFormsByUuid,
    dataConditions, viewDataConditions, hasMessageNode, approvalActions,
  ...
  if (hasDataNode && dataNodeId) {
    const resolvedDataFormName = dataFormName || dataSourceFormName || '';
    const dataSourceFormType = resolveDataSourceFormType(
      { formUuid: dataFormUuid, formType: dataFormType },
      {
        defaultFormType: dataFormType,
        defaultFormName: resolvedDataFormName,
        dataSourceFormsByUuid,
        formEventTypes,
      },
    );
    const resolvedDataSourceFormName = resolveDataSourceFormName(
      { formUuid: dataFormUuid, formName: resolvedDataFormName },
      { defaultFormName: resolvedDataFormName, dataSourceFormsByUuid },
    );
    const rawConditions = viewDataConditions || dataConditions;
    const designerConditions = (rawConditions || []).map((condition) => {
      if (!condition) {
        return condition;
      }
      const designerFieldId = toDesignerDataQueryField(condition.bFieldId, dataSourceFormType);
      return designerFieldId === condition.bFieldId
        ? condition
        : { ...condition, bFieldId: designerFieldId };
    });
    const conditions = designerConditions && designerConditions.length > 0
      ? buildDataRetrieveCondition(designerConditions)
        ...
        getData: {
          ...
          targetItem: {
            ...
            formItem: {
              ...
              title: resolvedDataSourceFormName,
              fields: null,
              hasTableField: null,
            },
          },
