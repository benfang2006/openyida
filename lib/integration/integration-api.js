 ...
 *   - getLogicflowDetail：按 processCode 读取逻辑流设计详情
 */
...
/**
 * 按 processCode 读取逻辑流设计详情。
 *
 * 该接口在已观测租户中返回 view schema；这里不把它猜测为完整 runtime graph，
 * 仅作为控制面 read-after-write 的独立存在性证据。
 */
async function getLogicflowDetail(authRef, params) {
  const { appType, formUuid, processCode } = params;
  const response = await createYidaClient({ authRef }).get(
    `/alibaba/web/${appType}/query/simpleProcess/getProcess.json`,
    () => ({
  ...
  if (!response || !response.success) {
    const errorMsg = response ? response.errorMsg || JSON.stringify(response) : '请求失败';
    throw new Error(`查询逻辑流详情失败：${errorMsg}`);
  }
  return response;
}
