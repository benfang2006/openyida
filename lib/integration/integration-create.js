...
async function hydrateSpecConnectorSchemas(authRef, spec) {
  ...
  for (const node of nodes) {
    ...
    validateConnectorAssignmentsAgainstSchema(assignments, schema.inputs);
...
function assertSpecFlagCompatibility(spec, subArgs) {
  ...
  const structuralFlags = [
    ...
    '--connector-id', '--action-id', '--connector-assignment',
  ];
  const mixedFlag = structuralFlags.find((flag) => subArgs.includes(flag));
  if (mixedFlag) {
    throw new CliError(`${mixedFlag} 不能与 --spec 混用；请把对应节点完整写入 spec，避免参数被静默丢弃。`, {
  ...
  if (hasMessageDefaults && !hasSpecNodeType(spec, 'sendMessage')) {
    throw new CliError('通知参数与 --spec 混用时，spec 必须显式声明 sendMessage 节点。', {
      code: 'INTEGRATION_SPEC_MESSAGE_NODE_REQUIRED',
      details: { remoteWrites: 0 },
    });
  }
  if (subArgs.includes('--initiate-approval-initiator-user')
    && !hasSpecNodeType(spec, 'initiateApproval')) {
    throw new CliError('审批发起人参数与 --spec 混用时，spec 必须显式声明 initiateApproval 节点。', {
      code: 'INTEGRATION_SPEC_APPROVAL_NODE_REQUIRED',
      details: { remoteWrites: 0 },
    });
  }
}
...
async function run(args) {
  ...
  if (hasConnectorCallNode) {
    ...
    validateConnectorAssignmentsAgainstSchema(connectorAssignments, schema.inputs);
  ...
  if (integrationSpec) {
    await hydrateSpecConnectorSchemas(authRef, integrationSpec);
  }
  ...
  for (const specFormUuid of specInitiateApprovalFormUuids) {
    ...
    try {
      step(`获取 ${specFormUuid} 流程表单 Schema`);
      const schema = await getFormSchema(authRef, { appType, formUuid: specFormUuid.toString() });
      formSchemasByUuid.set(specFormUuid.toString(), schema);
      warn(t('integration.create_get_schema_ok', String(schema.length)));
    } catch (error) {
      throw new Error(`获取 ${specFormUuid} 流程表单 Schema 失败，已停止远端写入: ${error.message}`);
    }
  }
  ...
  if (integrationSpec) {
    const built = buildSpecProcessAndViewJson({
      ...
      userFields,
      formSchemasByUuid,
      formNamesByUuid: new Map(Array.from(initiateApprovalTargetFormsByUuid.entries())
        .map(([uuid, form]) => [uuid, form && form.formName ? form.formName : uuid])),
