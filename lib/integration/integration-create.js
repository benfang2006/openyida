...
const { normalizeSourceFormType } = require('./integration-form-metadata');
...
async function run(args) {
  ...
  if (connectorInputsPath) {
    try {
      ...
      if (!Array.isArray(parsed)) {
        throw new Error('--connector-inputs must point to a JSON array');
      }
      connectorInputsJson = parsed;
    } catch (err) {
      throw new Error(`读取 --connector-inputs 文件失败：${err.message}`);
    }
  }
  ...
  for (const descriptor of sourceFormDescriptors) {
    ...
    try {
      ...
      if (!explicitType) {
        throw new Error(`获取取数来源表单信息失败，已停止远端写入: ${error.message}`);
      }
      dataSourceForm = {
    ...
    const remoteFormType = normalizeSourceFormType(dataSourceForm && dataSourceForm.formType);
    if (explicitType && remoteFormType && explicitType !== remoteFormType) {
      throw new Error(
        `获取取数来源表单信息失败，显式 formType=${descriptor.formType} 与导航元数据 ${dataSourceForm.formType} 不一致，已停止远端写入`
      );
    }
    ...
    if (!dataSourceForm || !sourceFormType) {
      ...
      if (!explicitType) {
        throw new Error(`获取取数来源表单信息失败，${reason}，已停止远端写入`);
      }
      dataSourceForm = {
        formUuid: descriptor.formUuid,
        formName: descriptor.formName || descriptor.formUuid,
  ...
  const viewDataConditions = dataConditions.map((condition) => (
    dataSourceForm && normalizeSourceFormType(dataSourceForm.formType) === 'process'
      && (!getSelf || !getSelfQueryFieldOverride)
      && condition.bFieldId === 'pid'
