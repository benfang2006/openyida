...
async function run(args) {
  ...
  let dataSourceForm = null;
  if (dataFormUuid) {
    try {
      dataSourceForm = await findFormNavigationItem(authRef, appType, dataFormUuid.toString(), getFormNavigationItems);
    } catch (error) {
      warn(`获取取数来源表单信息失败，继续按默认表单类型处理: ${error.message}`);
    }
  }
  if (dataSourceForm && dataSourceForm.formUuid) {
    if (getSelf && !getSelfQueryFieldOverride) {
      getSelfQueryField = normalizeFormType(dataSourceForm.formType) === 'process'
        ? 'pid'
        : 'form_inst_id';
  ...
  const viewDataConditions = dataConditions.map((condition) => (
    getSelf && dataSourceForm && normalizeFormType(dataSourceForm.formType) === 'process'
