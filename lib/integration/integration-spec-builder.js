...
function addDeclaredAliasNodes(nodes, aliasMap) {
  function visit(list) {
    for (const node of asArray(list)) {
  ...
  function visit(nodes, inheritedUpstream) {
    ...
    for (const node of asArray(nodes)) {
      ...
      if (type === 'route') {
        ...
        for (const branch of asArray(node.branches || node.conditions)) {
          const branchNodes = collectBranchNodes(branch);
          visit(branchNodes, branchUpstream);
          addDeclaredAliasNodes(branchNodes, seen);
        }
      }
      const alias = getDeclaredAlias(node);
      if (alias) {
...
function assertSubTableSources(spec) {
