...
function buildDataRetrieveViewNode(node, nodeId, context) {
  ...
  const targetItem = isSubTableRetrieve(node)
    ? {
      deep: 0,
  ...
  const relativeItem = isSubTableRetrieve(node)
    ? {
      deep: 0,
      value: props.subSourceId,
      label: node.subSourceLabel || props.subSourceId,
    }
    : {};
  return {
...
function buildDataCreateViewNode(node, nodeId, context) {
  ...
  if (target.isSubTable) {
    addDataRules.targetItem = {
      deep: 0,
      value: target.formUuid,
      label: target.formName,
    };
    addDataRules.relativeItem = {
      deep: 0,
      value: target.subFormUuid,
      label: resolveTableFieldLabel(schemaComponents, target.subFormUuid, target.subFormLabel),
    };
  }
  return {
