...
function resolveSubTableParentFormUuid(node, spec, triggerFormUuid) {
  if (node.parentFormUuid) {
    return node.parentFormUuid;
  }
  const sourceNode = findSpecNodeByAlias(spec, getSourceAlias(node));
  if (!sourceNode) {
    return '';
  }
  const sourceType = normalizeNodeType(sourceNode.type || sourceNode.componentName);
  if (sourceType === 'getSelf') {
...
function buildDataRetrieveViewNode(node, nodeId, context) {
  ...
  if (isSubTableRetrieve(node)) {
    const sourceNode = context.aliasToSpecNode && context.aliasToSpecNode.get(getSourceAlias(node));
    const parentFormUuid = (sourceNode && (sourceNode.formUuid || sourceNode.sourceId)) || '';
...
function resolveCreateTarget(node, context) {
  if (isSubTableInsert(node)) {
    ...
    const sourceNode = (context.aliasToSpecNode && context.aliasToSpecNode.get(sourceAlias)) || {};
    const sourceType = normalizeNodeType(sourceNode.type || sourceNode.componentName);
    const parentFormUuid = node.parentFormUuid
      || sourceNode.formUuid
      || sourceNode.sourceId
      || sourceNode.targetFormUuid
      || (sourceType === 'getSelf' ? context.formUuid : '');
...
function buildDataCreateViewNode(node, nodeId, context) {
  ...
  const childList = target.isSubTable
  ...
  childList.forEach((item) => {
    componentOptionMap[item.fieldId] = item.componentOption || '[]';
  });
  ...
  if (target.isSubTable) {
    ...
    if (target.parentFormUuid) {
      // Match designer-saved AddDataNode: formItem.fields stays null so the
      // side panel fetches the real form schema and fills the sub-table select.
      targetItem.formItem = {
        formType: 'receipt',
        advanceProc: 'n',
      ...
      addDataRules.sourceId = target.parentFormUuid;
    ...
    addDataRules.inputs.tableList = tableFields.length > 0 ? tableFields : addDataRules.relativeList;
  }
  return {
