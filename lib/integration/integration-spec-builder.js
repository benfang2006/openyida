  ...
  function validateNodes(nodes) {
    for (const node of asArray(nodes)) {
      ...
      if (type === 'dataRetrieve' && asArray(node.conditions || node.dataConditions).length === 0) {
        throw new Error('dataRetrieve node conditions must be non-empty; use getSelf for the triggering record');
      }
      if (type === 'dataCreate') {
        ...
        if (asArray(node.assignments).length === 0) {
          throw new Error('dataCreate node assignments must be non-empty');
        }
      }
      if (type === 'dataUpdate') {
        ...
        if (assignments.length === 0) {
          throw new Error('dataUpdate node assignments must be non-empty');
        }
      ...
      if (type === 'initiateApproval') {
        if (!(node.formUuid || node.targetFormUuid)) {
          throw new Error('initiateApproval node formUuid is required');
        }
        ...
        if (assignments.length === 0) {
          throw new Error('initiateApproval node assignments must be non-empty');
        }
        const initiator = node.initiator;
        if (!initiator || !['current_user', 'select_user'].includes(initiator.type)) {
          throw new Error('initiateApproval node initiator must be current_user or select_user');
        }
        if (initiator.type === 'select_user' && !initiator.value) {
          throw new Error('initiateApproval select_user initiator value is required');
        }
      }
      if (type === 'connector') {
...
function resolveInitiator(node, context) {
  if (node.initiator && node.initiator.type === 'current_user') {
    if (!context.currentUserId) {
      throw new Error('initiateApproval current_user initiator requires an authenticated user');
    }
    return {
      type: 'select_user',
  ...
  throw new Error('initiateApproval node initiator is not configured');
}
...
function buildDataRetrieveProcessNode(node, nodeId, nextNodeId, context) {
  ...
  return {
    ...
    description: configuredDescription(node, `从[${node.formName || formUuid}]按${conditions.rules.length}个条件获取数据`),
    type: 'dataRetrieve',
    nodeId,
    prevId: '',
    nextId: [nextNodeId],
...
function buildDataCreateProcessNode(node, nodeId, nextNodeId, context) {
  ...
  return {
    ...
    description: configuredDescription(node, `向[${node.formName || formUuid}]写入${assignments.length}个字段`),
    type: 'dataCreate',
    nodeId,
    prevId: '',
    nextId: [nextNodeId],
...
function buildDataUpdateProcessNode(node, nodeId, nextNodeId, context) {
  ...
  return {
    ...
    description: configuredDescription(node, `更新上游数据的${assignments.length}个字段`),
    type: 'dataUpdate',
    nodeId,
    prevId: '',
    nextId: [nextNodeId],
...
function buildDataUpdateViewNode(node, nodeId, context) {
  ...
  return {
    ...
    props: {
      ...
      name: node.name || '更新数据',
      description: configuredDescription(node, `更新上游数据的${asArray(node.assignments || node.updateAssignments).length}个字段`),
...
function buildInitiateApprovalProcessNode(node, nodeId, nextNodeId, context) {
  ...
  const description = configuredDescription(node, `在[${formName}]中发起审批并写入${assignments.length}个字段`);
  return {
