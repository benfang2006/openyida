...
function validateIntegrationSpec(spec, fallbackEvents = ['insert']) {
  ...
  function validateNodes(nodes) {
    for (const node of asArray(nodes)) {
      ...
      if (!['getSelf', 'dataRetrieve', 'dataCreate', 'dataUpdate', 'sendMessage', 'connector', 'route'].includes(type)) {
      ...
      if (type === 'dataUpdate') {
        validateAssignments(asArray(node.assignments || node.updateAssignments), 'dataUpdate assignment');
