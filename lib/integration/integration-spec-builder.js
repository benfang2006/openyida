...
const { buildAddDataChildList, assertAssignmentFieldsExist } = require('./integration-view-builder');
const {
...
function resolveDeclaredSourceFormType(node = {}) {
  const declarations = [
    ['formType', node.formType],
    ['sourceFormType', node.sourceFormType],
    ...
    .map(([field, value]) => ({
      field,
      rawValue: value,
      normalized: normalizeSourceFormType(value),
    }));
  if (declarations.length === 0) {
    return { rawValue: undefined, normalized: '' };
  }
  const unsupported = declarations.find((declaration) => !declaration.normalized);
  if (unsupported) {
    throw new Error(`Unsupported source form type: ${unsupported.rawValue}`);
  }
  const normalizedTypes = new Set(declarations.map((declaration) => declaration.normalized));
  if (normalizedTypes.size > 1) {
    throw new Error(
      `Conflicting source form types on node: ${declarations.map((declaration) => (
        `${declaration.field}=${declaration.rawValue}`
      )).join(', ')}`
    );
  }
  return {
    rawValue: declarations[0].rawValue,
    normalized: declarations[0].normalized,
  };
}
...
function normalizeDataConditions(conditions, context, sourceFormType) {
  return asArray(conditions).map((condition) => {
    ...
    const bFieldName = condition.bFieldName || condition.fieldName || condition.name || condition.label || bFieldId;
    return {
      bFieldId: resolveNodeRefs(bFieldId, context),
      bFieldName: normalizeDataQueryFieldName(bFieldId, sourceFormType, bFieldName),
...
function buildDataRetrieveProcessNode(node, nodeId, nextNodeId, context) {
  ...
  const dataSourceFormType = resolveDataSourceFormType(node, context);
  const resolvedSourceFormName = resolveDataSourceFormName(node, context);
  const formName = resolvedSourceFormName
    || node.formName
    || (context.formNamesByUuid && context.formNamesByUuid.get(formUuid))
    || formUuid;
  const rawConditions = node.type === 'getSelf' || node.getSelf
    ? [{
      bFieldId: resolveDataQueryField(node, dataSourceFormType),
      bFieldName: resolveDataQueryFieldName(dataSourceFormType),
      aFieldId: node.triggerField || '__masterdata_form_inst_id',
      componentType: 'TextField',
      opCode: 'Equal',
      valueType: 'processVar',
    }]
    : normalizeDataConditions(node.conditions || node.dataConditions, context, dataSourceFormType);
...
function buildDataRetrieveViewNode(node, nodeId, context) {
  ...
  const dataSourceFormType = resolveDataSourceFormType(node, context);
  const dataSourceFormName = resolveDataSourceFormName(node, context);
  let viewCondition;
  if (node.type === 'getSelf' || node.getSelf) {
    viewCondition = buildDataRetrieveCondition([{
      bFieldId: resolveDesignerDataQueryField(node, dataSourceFormType),
      bFieldName: resolveDataQueryFieldName(dataSourceFormType),
      aFieldId: node.triggerField || '__masterdata_form_inst_id',
      componentType: 'TextField',
      opCode: 'Equal',
    ...
    viewCondition = {
      ...props.condition,
      rules: (props.condition.rules || []).map((rule) => {
        const designerFieldId = toDesignerDataQueryField(rule.id, dataSourceFormType);
        const designerFieldName = normalizeDataQueryFieldName(
          rule.id,
          dataSourceFormType,
          rule.name,
        );
        return designerFieldId === rule.id && designerFieldName === rule.name
          ? rule
          : {
            ...rule,
            id: designerFieldId,
  ...
  const formName = dataSourceFormName
    || node.formName
    || (context.formNamesByUuid && context.formNamesByUuid.get(props.sourceId))
    || props.sourceId;
  return {
    ...
    props: {
      ...
      getData: {
        ...
        targetItem: {
          ...
          appName: '',
          formItem: {
            formType: resolveDataFormItemType(dataSourceFormType),
            advanceProc: 'n',
            formUuid: props.sourceId,
            title: dataSourceFormName || formName,
            fields: null,
            hasTableField: null,
          },
        },
...
function buildDataCreateProcessNode(node, nodeId, nextNodeId, context) {
  ...
  return {
    ...
    description: configuredDescription(node, t('integration.spec_desc_create', formName, assignments.length)),
    type: 'dataCreate',
    nodeId,
    prevId: '',
    nextId: [nextNodeId],
...
function buildDataCreateViewNode(node, nodeId, context) {
  ...
  assertAssignmentFieldsExist(normalizedAssignments, childList);
  ...
  const description = node.description || `在[${formName}]中新增 ${assignmentRules.length} 个字段`;
  return {
...
function buildSpecProcessAndViewJson(options) {
  ...
  // Validate all source declarations, including duplicate UUIDs, before any
  // caller can serialize the generated payload for a remote write.
  collectDataSourceFormDescriptors(spec, options.formUuid);
  ...
  const context = {
    ...
    defaultFormType: options.dataFormType || options.formType || '',
    defaultFormName: options.dataFormName || options.dataSourceFormName || options.formName || '',
    dataSourceForm: options.dataSourceForm || null,
    dataSourceFormsByUuid: options.dataSourceFormsByUuid || new Map(),
...
function collectDataSourceFormDescriptors(spec, triggerFormUuid) {
  ...
  function visit(nodes) {
    for (const node of asArray(nodes)) {
      ...
      const type = normalizeNodeType(node.type || node.componentName);
      if (type === 'getSelf' || type === 'dataRetrieve') {
        const declaredType = resolveDeclaredSourceFormType(node);
        const formUuid = resolveSourceFormUuid({ ...node, type }, { formUuid: triggerFormUuid });
        if (formUuid) {
          const key = String(formUuid);
          const existing = result.get(key) || { formUuid: key };
          if (declaredType.normalized) {
            if (existing.formType
              && normalizeSourceFormType(existing.formType) !== declaredType.normalized) {
              throw new Error(
                `Conflicting source form types for ${key}: ${existing.formType} and ${declaredType.rawValue}`
              );
            }
            existing.formType = declaredType.normalized;
          }
          if (node.formName || node.sourceFormName || node.dataFormName) {
            existing.formName = node.formName || node.sourceFormName || node.dataFormName;
          }
