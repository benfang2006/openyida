...
function assertActionRevisionAdvanced(action, beforeConfig, responseContent, readbackConfig) {
  ...
  const verifiedRevision = revisionSource === 'response' ? responseRevision : readbackRevision;

  if (isMissingRevision(verifiedRevision)) {
    const error = new Error('AGGREGATE_WRITE_REVISION_MISSING');
    error.code = 'AGGREGATE_WRITE_REVISION_MISSING';
    throw error;
  }
  if (!isMissingRevision(beforeRevision) && String(verifiedRevision) === String(beforeRevision)) {
    const errorCode = revisionSource === 'response'
      ? 'AGGREGATE_WRITE_RESPONSE_REVISION_UNCHANGED'
      : 'AGGREGATE_WRITE_REVISION_UNCHANGED';
