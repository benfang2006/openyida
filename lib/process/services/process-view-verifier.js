...
function parseJsonValue(value, label) {
  ...
  try {
    ...
    const wrapped = new TypeError(label + ' must be valid JSON: ' + error.message);
...
function extractPlatformView(response) {
  if (!response || response.success !== true) {
    const error = new TypeError('getProcessById did not return a successful response');
    error.code = 'PROCESS_PLATFORM_VIEW_REQUEST_FAILED';
    throw error;
  }
  let payload = parseJsonValue(response.content, 'getProcessById content');
  if (isPlainObject(payload) && isPlainObject(payload.data)) {
    payload = payload.data;
  }
  if (!isPlainObject(payload)) {
    const error = new TypeError('getProcessById content must resolve to an object');
  ...
  if (!isPlainObject(payload.schema) || payload.schema.componentName !== 'CanvasEngine') {
    const error = new TypeError('platform view must contain a CanvasEngine schema');
    error.code = 'PROCESS_PLATFORM_VIEW_SCHEMA_MISSING';
    throw error;
  }
  if (!Array.isArray(payload.schema.children)) {
    const error = new TypeError('platform view schema children must be an array');
