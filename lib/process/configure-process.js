...
async function run(args, options) {
  ...
  let versionState = { published: null, saved: null };
  if (!startedAsReceipt) {
  ...
  if (savedDraft) {
    ...
    if (!newProcessId) {
      const refreshedSaved = await queryProcessVersions(
        authRef, appType, processCode, 'SAVED', { oneShot: true }
      );
      const refreshedDraft = latestProcessVersion(refreshedSaved);
      if (refreshedDraft) {
        newProcessId = rawProcessId(refreshedDraft);
        newVersion = versionNumber(refreshedDraft) || newVersion;
      }
    }
  }
