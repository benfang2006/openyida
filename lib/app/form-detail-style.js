...
const YIDA_GLOBAL_THEME_STYLE_ID = 'yida-global-theme';
const FORM_DETAIL_STYLE_ID = 'yida-form-detail-style';
const STYLE_MARKER = 'yida-form-detail';
const GLOBAL_THEME_MARKER = 'openyida:yida-global-theme';
const ROOT_CSS_START = '/* openyida:yida-form-detail:start */';
const ROOT_CSS_END = '/* openyida:yida-form-detail:end */';
const DEFAULT_PRESET = 'clean-card';

const DEFAULT_YIDA_GLOBAL_THEME_CSS = `/* ${GLOBAL_THEME_MARKER}:form-detail */
:root {
  --color-brand1-6: rgba(57, 84, 228, 1);
  --color-brand-4: rgba(40, 59, 160, 1);
  --color-brand1-2: rgba(243, 245, 253, 1);
...
const USAGE = '用法: openyida form-detail-style <apply|remove|check> <appType> <formUuid> [--css file] [--preset clean-card] [--json]';
const EXAMPLE = '示例: openyida form-detail-style apply APP_XXX FORM_XXX --preset clean-card';
...
function findNode(node, matcher) {
  if (!node) {
    return null;
  }
  if (matcher(node)) {
  ...
  for (let i = 0; i < children.length; i += 1) {
    const found = findNode(children[i], matcher);
    if (found) {
      return found;
    }
  }
...
function findNodeById(node, id) {
  return findNode(node, item => item && item.id === id);
}
...
function findFormContainer(schema) {
  ...
  return findNode(rootNode, item => item && item.componentName === 'FormContainer');
}

function buildStyleContent(css) {
  const safeThemeCss = DEFAULT_YIDA_GLOBAL_THEME_CSS.replace(/<\/style/gi, '<\\/style');
  const safeCss = normalizeCssSource(css).replace(/<\/style/gi, '<\\/style');
  return `<style id="${YIDA_GLOBAL_THEME_STYLE_ID}">${safeThemeCss}</style>\n<style id="${FORM_DETAIL_STYLE_ID}">${safeCss}</style>`;
}

function buildRootCssContent(css) {
  return `${DEFAULT_YIDA_GLOBAL_THEME_CSS}\n\n${normalizeCssSource(css)}`;
}

function removeRootCssBlock(css = '') {
  const source = String(css || '');
  const pattern = new RegExp(`${escapeRegExp(ROOT_CSS_START)}[\\s\\S]*?${escapeRegExp(ROOT_CSS_END)}\\n?`, 'g');
...
function upsertRootCssBlock(rootNode, css) {
  const existing = removeRootCssBlock(rootNode.css || '');
  const block = `${ROOT_CSS_START}\n${buildRootCssContent(css)}\n${ROOT_CSS_END}`;
  rootNode.css = [existing, block].filter(Boolean).join('\n\n');
}

function removeInjectedNodes(children) {
  let removed = 0;
  for (let i = children.length - 1; i >= 0; i -= 1) {
    const item = children[i];
    if (item && item.id === FORM_DETAIL_HTML_ID) {
      children.splice(i, 1);
      removed += 1;
      continue;
    }
    const childNodes = readNodeChildren(item);
    if (childNodes.length > 0) {
      removed += removeInjectedNodes(childNodes);
    }
  }
  return removed;
}
...
function upsertFormDetailCss(schema, css) {
  ...
  if (!formContainer) {
    throwCommandError('Schema 中未找到 FormContainer');
  }

  upsertRootCssBlock(rootNode, css);

  const children = ensureNodeChildren(formContainer);
  const content = buildStyleContent(css);
  const existing = children.find(item => item && item.id === FORM_DETAIL_HTML_ID);
  const baseNode = existing || {};

  baseNode.componentName = 'Html';
  baseNode.id = FORM_DETAIL_HTML_ID;
  baseNode.props = Object.assign({}, baseNode.props, {
    content,
    __style__: {
      height: '0px',
      overflow: 'hidden',
  ...
  baseNode.title = '勿删:详情页CSS';
  baseNode.isLocked = true;
  baseNode.condition = true;
  baseNode.conditionGroup = '';

  if (existing) {
    return 'updated';
  }

  children.unshift(baseNode);
  return 'inserted';
}

function removeFormDetailCss(schema) {
  const rootNode = findRootNode(schema);
  if (!rootNode) {
    throwCommandError('Schema 中未找到 RootContent');
  }
  rootNode.css = removeRootCssBlock(rootNode.css || '');
  const removed = removeInjectedNodes(readNodeChildren(rootNode));
  return removed > 0 ? 'removed' : 'absent';
}

function inspectFormDetailCss(schema) {
  const rootNode = findRootNode(schema);
  const htmlNode = rootNode ? findNodeById(rootNode, FORM_DETAIL_HTML_ID) : null;
  const content = htmlNode && htmlNode.props ? String(htmlNode.props.content || '') : '';
  const rootCss = rootNode ? String(rootNode.css || '') : '';
  return {
    installed: !!htmlNode && htmlNode.componentName === 'Html' && htmlNode.hidden === false && content.includes(STYLE_MARKER),
    htmlComponentFound: !!htmlNode,
    htmlComponentId: FORM_DETAIL_HTML_ID,
    componentName: htmlNode ? htmlNode.componentName : '',
    hidden: htmlNode ? !!htmlNode.hidden : null,
    contentHasMarker: content.includes(STYLE_MARKER),
    globalThemeFound: content.includes(`id="${YIDA_GLOBAL_THEME_STYLE_ID}"`) || content.includes(`id='${YIDA_GLOBAL_THEME_STYLE_ID}'`),
    rootCssHasGlobalTheme: rootCss.includes(GLOBAL_THEME_MARKER),
    rootCssHasMarker: rootCss.includes(ROOT_CSS_START) && rootCss.includes(STYLE_MARKER),
  };
}
...
async function applyStyle(authRef, parsed) {
  ...
  const action = upsertFormDetailCss(schema, css);
  ...
  return { success: true, action, installed: true, saveResult, configResult };
}
...
async function removeStyle(authRef, parsed) {
  ...
  const version = extractSchemaServerRevision(schemaResult);
  const schema = extractSchema(schemaResult);
  const action = removeFormDetailCss(schema);
  const saveResult = await saveFormSchema(authRef, parsed.appType, parsed.formUuid, schema, version);
  assertSaveSuccess(saveResult);
  const configResult = await refreshFormConfig(authRef, parsed.appType, parsed.formUuid, version);
...
module.exports = {
  ...
  _private: {
    FORM_DETAIL_HTML_ID,
    FORM_DETAIL_FIELD_ID,
    YIDA_GLOBAL_THEME_STYLE_ID,
