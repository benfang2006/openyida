...
// Pod profile names are platform app theme keys and the default page-level theme suggestions.
// Legacy basic token profile names (blue / green / orange) remain accepted for older specs.
...
function registerTokenThemeProfiles(presetKeys, tokenPresets, options = {}) {
  presetKeys.forEach((presetName) => {
    ...
    DEFAULT_THEME_PROFILES[presetName] = {
      name: presetName,
      appThemeKey: options.appThemeKey === false ? '' : presetName,
...
registerTokenThemeProfiles(POD_THEME_TOKEN_PRESET_KEYS, POD_THEME_TOKEN_PRESETS, {
  themeColorSource: 'platform-pod-theme',
});

registerTokenThemeProfiles(BASIC_THEME_TOKEN_PRESET_KEYS, BASIC_THEME_TOKEN_PRESETS, {
  appThemeKey: false,
