...
const DEFAULT_THEME_PROFILE_KEY = 'blue';
...
// Basic token profile names are blue / green / orange; they are not app --theme keys.
...
BASIC_THEME_TOKEN_PRESET_KEYS.forEach((presetName) => {
  const tokens = BASIC_THEME_TOKEN_PRESETS[presetName];
...
function normalizeAppBlueprint(spec, scene, templateName, meta) {
  ...
  return {
    ...
    navConfig: {
      ...
      reason: hideAppNav
        ? 'explicit page-level navigation should own the viewport'
