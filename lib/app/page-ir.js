...
const DEFAULT_THEME_PROFILE_KEY = 'podBlue';
...
// Application theme token profiles work as the default page-level theme suggestions.
// Only platform-supported theme keys are mirrored into appThemeKey.
...
function registerTokenThemeProfiles(presetKeys, tokenPresets, options = {}) {
  presetKeys.forEach((presetName) => {
    const tokens = tokenPresets[presetName];
    DEFAULT_THEME_PROFILES[presetName] = {
      name: presetName,
      appThemeKey: options.appThemeKeyFor ? options.appThemeKeyFor(presetName) : (options.appThemeKey === false ? '' : presetName),
      followRuntimeTheme: false,
      themeColorSource: options.themeColorSource || 'theme-token-preset',
      themeColor: tokens['--color-brand1-6'],
      themeColorDeep: tokens['--color-brand1-9'],
      themeColorSoft: tokens['--color-brand1-2'],
      themeColorTint: tokens['--color-brand1-3'],
      navTheme: 'light',
      mode: 'color_color',
      colorMode: 'gradient',
      mobileNavStyle: 'top',
...
registerTokenThemeProfiles(APP_THEME_TOKEN_PRESET_KEYS, APP_THEME_TOKEN_PRESETS, {
  themeColorSource: 'application-theme',
  appThemeKeyFor: (presetName) => (isPresetThemeKey(presetName) ? presetName : ''),
});
...
function normalizeThemeProfile(rawProfile) {
  const defaults = DEFAULT_THEME_PROFILES[DEFAULT_THEME_PROFILE_KEY];
  const profile = typeof rawProfile === 'string'
    ? { name: rawProfile }
    : isPlainObject(rawProfile)
      ? rawProfile
  ...
  const isKnownProfile = Boolean(DEFAULT_THEME_PROFILES[name]);
  const normalizedName = isKnownProfile ? preset.name : name;
  ...
  const normalized = {
    name: normalizedName,
    followRuntimeTheme,
    themeColorSource: followRuntimeTheme
      ? 'runtime-css-vars'
      : valueOrDefault(
        profile.themeColorSource || profile.colorSource,
        hasExplicitColorFamily ? 'profile' : (preset.themeColorSource || 'profile')
      ),
  ...
  const appThemeKey = valueOrDefault(profile.appThemeKey || profile.themeKey, isKnownProfile ? preset.appThemeKey : '');
  if (appThemeKey) {
    normalized.appThemeKey = appThemeKey;
  }
...
function inferHideAppNav(source) {
...
function normalizeAppBlueprint(spec, scene, templateName, meta) {
  ...
  const hideAppNav = inferHideAppNav(source);
  ...
  return {
    ...
    navConfig: {
      ...
      reason: hideAppNav
        ? 'explicit hide navigation request'
...
function normalizeMediaAssets(spec, defaults) {
  ...
  return {
    ...
    storyImage: valueOrDefault(source.storyImage || source.story || spec.storyImage || spec.story_image, fallback.storyImage || ''),
    storyImageAlt: valueOrDefault(source.storyImageAlt || source.storyAlt || spec.storyImageAlt || spec.story_image_alt, fallback.storyImageAlt || ''),
