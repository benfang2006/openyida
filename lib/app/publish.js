 ...
 * 3. 读取本地 token session 获取登录态；若未登录或 token 失效，则提示重新执行 openyida login
...
// 与普通自定义页面 Schema 的差异仅在 content JSON：Page.children 直接挂一个 YidaCodeCanvas
...
async function sendHealthCheckRequest(pageUrl) {
...
async function main(argv) {
  ...
  // 路由：默认普通自定义页面；.canvas.jsx / .canvas.tsx 源文件自动走 Code Canvas 链路，
  ...
  if (isCanvas) {
    // Code Canvas 链路：跳过普通自定义页面编译/lint，改用本地 Babel 编译
