...
async function runMain(argv) {
  ...
  if (autoNavOrder) {
    step(healthCheck ? 6 : 5, t('publish.step_nav_order'));
    try {
  ...
  if (configResponse && configResponse.success) {
    ...
    console.log(JSON.stringify(withBrowserHandoff(
      { success: true, appType, formUuid: savedFormUuid, version, url: pageUrl, configWarning: configErrorMsg, healthCheck: healthCheckResult, navOrder: navOrderResult, navOrderWarning },
      pageUrl,
      { stage: 'publish_page_success', title: savedFormUuid },
      browserOpenMode
    )));
