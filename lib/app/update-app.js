 ...
 *
 * 用法：openyida update-app <appType> --name "新名称" [--desc "描述"] [--icon "图标"] [--layout slide|ver] [--hide-app-nav]
 */
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case '--layoutDirection':
        ...
        break;
      case '--hide-app-nav':
        result.hideAppNav = true;
        break;
      case '--show-app-nav':
        result.hideAppNav = false;
        break;
    }
  }
...
function hasShellUpdate(params) {
  return !!(params.icon || params.colour || params.navTheme || params.layoutDirection || params.hideAppNav !== null);
}
...
function buildUpdateAppPostData(params, currentApp = {}, authRef) {
  ...
  if (params.hideAppNav !== null) {
    postDataObj.hideAppNav = params.hideAppNav ? 'y' : 'n';
  }
...
async function run(args) {
  ...
  if (params.hideAppNav !== null) {
    label('Hide App Nav', params.hideAppNav ? 'y' : 'n');
  }
