...
async function readFormMode(context, input, options = {}) {
  ...
  const result = await dispatchRemotePrimitive(context, () => queryFormProcessBinding(
    ...
    { oneShot: options.oneShot === true || hasRemoteDispatchBoundary(context) }
  ));
...
async function getProcessCodeFromAppParam(authRef, appType, formUuid, options = {}) {
  // Keep the legacy helper name for callers; the implementation now uses the exact form binding endpoint.
  return getProcessCodeFromFormBinding(authRef, appType, formUuid, options);
}

async function getProcessCodeFromFormBinding(authRef, appType, formUuid, options = {}) {
  const result = await queryFormProcessBinding(authRef, appType, formUuid, options);
