...
function collectTextDeep(value, output) {
  ...
  if (Array.isArray(value)) {
    value.forEach((item) => collectTextDeep(item, output));
    return output;
  }
  if (typeof value === 'object') {
    Object.keys(value).forEach((key) => collectTextDeep(value[key], output));
  }
  return output;
}
...
function collectSpecText(spec) {
  ...
  return collectTextDeep(spec, []).join(' ');
}

function collectRoutingText(spec) {
  if (!spec || typeof spec !== 'object') {
    return '';
  }
  return [
...
function stripFallbackSource(source) {
  return String(source || '')
    .replace(/'data:image\/[^']*'/g, "''")
    .replace(/"data:image\/[^"]*"/g, '""')
    .replace(/const\s+[A-Z0-9_]*IMAGE\s*=\s*['"][\s\S]*?['"];\n/g, '')
    .replace(/applyPageFallbacks\(\{[\s\S]*?\n\}\);\n/g, '')
    .replace(/const\s+DEFAULT_[A-Z0-9_]+\s*=\s*[\s\S]*?;\n/g, '');
}

function findSampleResidues(source, templateName, spec) {
  const tokens = SAMPLE_RESIDUE_TOKENS[templateName] || [];
  if (!tokens.length) {
    return [];
  }
  const searchableSource = normalizeSearchText(stripFallbackSource(source));
  const specText = normalizeSearchText(collectSpecText(spec || {}));
  const residues = tokens.filter((token) => {
    const normalizedToken = normalizeSearchText(token);
    return searchableSource.includes(normalizedToken) && !specText.includes(normalizedToken);
  });
  return Array.from(new Set(residues));
}

function assertNoSampleResidueInDomainReadyOutput(source, templateName, spec, ir) {
  const fidelity = ir && ir.domainFidelity;
  if (!fidelity || fidelity.status !== 'domain-ready') {
    return;
  }
  const residues = findSampleResidues(source, templateName, spec);
  if (!residues.length) {
    return;
  }
  fidelity.status = 'draft-needs-domain-spec';
  fidelity.sampleFallbacks = Array.from(new Set([...(fidelity.sampleFallbacks || []), ...residues.map((item) => `source:${item}`)]));
  throwCommandError(`生成页仍残留 sample 内容，已停止写入/发布：${residues.join(', ')}。请把这些文案、行业词、配图说明或样式命名替换为当前业务自己的内容。`);
}
...
function inferTemplateName(options, spec) {
  ...
  const text = collectRoutingText(spec);
...
function reportNavigationConfig(ir) {
  ...
  hint('检测到显式隐藏平台导航，发布后应设置 isRenderNav=false：openyida update-form-config <appType> <formUuid> false "<页面标题>"');
}
...
async function run(args) {
  if ((args || []).includes('--help') || (args || []).includes('-h')) {
    console.log(t('cli.generate_page_usage'));
    console.log(t('cli.generate_page_example'));
    return;
  }
  ...
  assertNoSampleResidueInDomainReadyOutput(outputSource, templateName, spec, ir);
  ...
  if (mode === 'canvas') {
    ...
    result(true, t('generate_page.success'), [
      ...
      ['Design profile', ir.visualProfile && ir.visualProfile.name],
      ['Theme profile', ir.themeProfile && ir.themeProfile.name],
      ['Theme source', ir.themeProfile && ir.themeProfile.themeColorSource],
      ['Theme color', ir.themeProfile && ir.themeProfile.themeColor],
      ['Nav theme', ir.themeProfile && (ir.themeProfile.navTheme || 'light')],
      ['Theme color mode', ir.themeProfile && ir.themeProfile.colorMode],
...
module.exports = {
  ...
  inferTemplateName,
  findSampleResidues,
  assertNoSampleResidueInDomainReadyOutput,
};
