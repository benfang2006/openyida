...
function getCanvasDistPath(outputPath) {
  const parsed = path.parse(outputPath);
  const pagesIndex = parsed.dir.split(path.sep).lastIndexOf('src');
  if (pagesIndex >= 0) {
    const parts = parsed.dir.split(path.sep);
    parts[pagesIndex] = 'dist';
    return path.join(parts.join(path.sep), `${parsed.name}.js`);
  }
  return path.join(parsed.dir, 'dist', `${parsed.name}.js`);
}
