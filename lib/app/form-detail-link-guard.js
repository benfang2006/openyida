...
function findFormDetailLinkIssues(sourceCode, options = {}) {
  ...
  traverse(ast, {
    ObjectExpression(pathRef) {
      ...
      if (expressionMayBeEmptyFallback(formInstIdProperty.value, sourceCode)) {
        issues.push({
          type: 'empty-formInstId-fallback',
          line: getNodeLine(formInstIdProperty.value),
        });
      }
      if (!expressionUsesPrimaryFormInstId(formInstIdProperty.value, sourceCode)) {
        issues.push({
...
function formatFormDetailLinkMessage(issue) {
  ...
  if (issue && issue.type === 'missing-primary-formInstId') {
    return '表单详情页实例 ID 必须优先使用 row.formInstId。可以保留 formInstanceId / instanceId / id 作为兼容兜底，但不能漏掉 formInstId。';
  }
  return '表单详情页实例 ID 必须优先使用 row.formInstId，并在缺失时阻止打开详情页。';
}
