...
const { buildApiPath } = require('./create-form/schema-builder');
...
function collectFieldDescriptors(fields, output, aliasByFieldId) {
  ...
  fields.forEach(function (field) {
    ...
    if (fieldId && !isFormPresentationComponent(field.componentName)) {
      descriptors.push({
        fieldId,
...
const DIVIDER_THEME_BLOCK_START = '/* openyida:divider-theme:start */';
const DIVIDER_THEME_BLOCK_END = '/* openyida:divider-theme:end */';
const DIVIDER_THEME_DID_MOUNT = 'openyidaDividerThemeDidMount';

function schemaHasComponentName(node, componentName) {
  ...
  return Array.isArray(node.children) && node.children.some(function (child) {
    return schemaHasComponentName(child, componentName);
  });
}

function schemaContainsDivider(schema) {
  const root = schema &&
    schema.pages &&
    schema.pages[0] &&
    schema.pages[0].componentsTree &&
    schema.pages[0].componentsTree[0];
  return schemaHasComponentName(root, 'Divider');
}
...
function buildDividerThemeActionSource(previousActionName) {
  const previousName = isValidActionIdentifier(previousActionName) && previousActionName !== DIVIDER_THEME_DID_MOUNT
    ? previousActionName
    : 'didMount';
  return `
function openyidaDividerThemeGetWindowDocuments() {
  var docs = [];
...
function openyidaDividerThemeParseThemeName(doc) {
...
function openyidaDividerThemeColorToRgb(color) {
...
function openyidaDividerThemeRgba(rgb, alpha) {
  return 'rgba(' + rgb.r + ', ' + rgb.g + ', ' + rgb.b + ', ' + alpha + ')';
}

function openyidaDividerThemeMix(rgb, opacity, bg) {
...
function openyidaDividerThemeReadBrand(doc) {
  try {
...
function openyidaDividerThemeResolveColor(doc) {
  var fromVar = openyidaDividerThemeReadBrand(doc);
  if (fromVar) {
    return fromVar;
  }
  var themeName = openyidaDividerThemeParseThemeName(doc);
  var themeColors = {
    podBlue: '#007FFF',
    blue: '#007FFF',
    teal: '#009595',
...
function openyidaDividerThemeBuildVariables(brandColor) {
  var rgb = openyidaDividerThemeColorToRgb(brandColor);
  if (!rgb) {
    return {
  ...
  var deep = openyidaDividerThemeMix(rgb, 0.7, 'black');
  var light = openyidaDividerThemeMix(rgb, 0.7, 'white');
  var soft = openyidaDividerThemeMix(rgb, 0.06, 'white');
  return {
    '--color-brand1-6': openyidaDividerThemeRgba(rgb, 1),
    '--color-brand-4': openyidaDividerThemeRgba(deep, 1),
    '--color-brand1-2': openyidaDividerThemeRgba(soft, 1),
    '--color-brand-3': openyidaDividerThemeRgba(rgb, 1),
    '--color-brand1-1': openyidaDividerThemeRgba(light, 1),
    '--color-brand-2': openyidaDividerThemeRgba(light, 1),
    '--color-brand-1': openyidaDividerThemeRgba(rgb, 0.3),
    '--color-brand1-3': openyidaDividerThemeRgba(rgb, 0.2),
    '--color-group': openyidaDividerThemeRgba(rgb, 1) + ',rgba(0, 122, 255, 1),rgba(138, 92, 253, 1),rgba(30, 41, 128, 1),rgba(255, 107, 53, 1),rgba(0, 200, 255, 1)',
    '--color-brand1-10': openyidaDividerThemeRgba(rgb, 0.3),
    '--color-brand1-9': openyidaDividerThemeRgba(deep, 1)
  };
}

function openyidaDividerThemeInjectIntoDocument(doc) {
  if (!doc || !doc.head || !doc.documentElement) {
    return;
  }
  if (doc.getElementById('yida-global-theme')) {
    return;
  }
  var themeConfig = openyidaDividerThemeBuildVariables(openyidaDividerThemeResolveColor(doc));
...
export function openyidaInjectDividerTheme() {
  openyidaDividerThemeGetWindowDocuments().forEach(openyidaDividerThemeInjectIntoDocument);
}

export function ${DIVIDER_THEME_DID_MOUNT}(event) {
  if (typeof ${previousName} === 'function') {
    ${previousName}.call(this, event);
  }
  return openyidaInjectDividerTheme();
}
`;
}

function ensureDividerThemeAction(schema) {
  if (!schemaContainsDivider(schema)) {
    return false;
  }
  ...
  const previousActionName = currentLifeCycle && currentLifeCycle.type === 'actionRef'
    ...
    : 'didMount';
  const generatedSource = buildDividerThemeActionSource(previousActionName);

  root.lifeCycles = root.lifeCycles || {};
  root.lifeCycles.componentDidMount = {
    name: DIVIDER_THEME_DID_MOUNT,
    id: DIVIDER_THEME_DID_MOUNT,
    params: {},
    type: 'actionRef',
  };
  ...
  actions.module.source = upsertGeneratedSourceBlockWithBounds(
    ...
    generatedSource,
    DIVIDER_THEME_BLOCK_START,
    DIVIDER_THEME_BLOCK_END
  );
...
function applyChangesToSchema(schema, changes) {
  ...
  changes.forEach(function (change, changeIndex) {
    ...
    if (change.action === 'add') {
      ...
      appliedChanges.push({ action: 'add', label: displayName, type: componentName });
      ...
      const deleteResult = findFieldByLabelDeep(formFields, change.label);
      if (deleteResult) {
