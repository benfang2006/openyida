...
const FORM_DETAIL_STYLE_HTML_ID = 'yida-form-detail-css-html';
const FORM_DETAIL_STYLE_FIELD_ID = 'html_yida_detail_css';
...
function isFormDetailStyleNode(component) {
  return !!component && (
    component.id === FORM_DETAIL_STYLE_HTML_ID ||
    getFieldIdFromComponent(component) === FORM_DETAIL_STYLE_FIELD_ID
  );
}
...
function collectFieldReferences(fields, options) {
  ...
  function visit(items, ancestors, pathLabels) {
    ...
    items.forEach(function (field, fieldIndex) {
      if (isFormDetailStyleNode(field)) {
        return;
      }
...
const YIDA_GLOBAL_THEME_BLOCK_START = '/* openyida:yida-global-theme:start */';
const YIDA_GLOBAL_THEME_BLOCK_END = '/* openyida:yida-global-theme:end */';
const YIDA_GLOBAL_THEME_DID_MOUNT = 'openyidaGlobalThemeDidMount';
const LEGACY_DIVIDER_THEME_BLOCK_START = '/* openyida:divider-theme:start */';
const LEGACY_DIVIDER_THEME_BLOCK_END = '/* openyida:divider-theme:end */';
const LEGACY_DIVIDER_THEME_DID_MOUNT = 'openyidaDividerThemeDidMount';
...
function buildYidaGlobalThemeActionSource(previousActionName) {
  const previousName = isValidActionIdentifier(previousActionName) &&
    previousActionName !== YIDA_GLOBAL_THEME_DID_MOUNT &&
    previousActionName !== LEGACY_DIVIDER_THEME_DID_MOUNT
    ? previousActionName
    : 'didMount';
  return `
function openyidaGlobalThemeGetWindowDocuments() {
  var docs = [];
...
function openyidaGlobalThemeParseThemeName(doc) {
...
function openyidaGlobalThemeColorToRgb(color) {
...
function openyidaGlobalThemeRgba(rgb, alpha) {
  return 'rgba(' + rgb.r + ', ' + rgb.g + ', ' + rgb.b + ', ' + alpha + ')';
}

function openyidaGlobalThemeMix(rgb, opacity, bg) {
...
function openyidaGlobalThemeReadBrand(doc) {
  try {
...
function openyidaGlobalThemeResolveColor(doc) {
  var fromVar = openyidaGlobalThemeReadBrand(doc);
  if (fromVar) {
    return fromVar;
  }
  var themeName = openyidaGlobalThemeParseThemeName(doc);
  var themeColors = {
    podBlue: '#007FFF',
    blue: '#007FFF',
    teal: '#009595',
...
function openyidaGlobalThemeBuildVariables(brandColor) {
  var rgb = openyidaGlobalThemeColorToRgb(brandColor);
  if (!rgb) {
    return {
  ...
  var deep = openyidaGlobalThemeMix(rgb, 0.7, 'black');
  var light = openyidaGlobalThemeMix(rgb, 0.7, 'white');
  var soft = openyidaGlobalThemeMix(rgb, 0.06, 'white');
  return {
    '--color-brand1-6': openyidaGlobalThemeRgba(rgb, 1),
    '--color-brand-4': openyidaGlobalThemeRgba(deep, 1),
    '--color-brand1-2': openyidaGlobalThemeRgba(soft, 1),
    '--color-brand-3': openyidaGlobalThemeRgba(rgb, 1),
    '--color-brand1-1': openyidaGlobalThemeRgba(light, 1),
    '--color-brand-2': openyidaGlobalThemeRgba(light, 1),
    '--color-brand-1': openyidaGlobalThemeRgba(rgb, 0.3),
    '--color-brand1-3': openyidaGlobalThemeRgba(rgb, 0.2),
    '--color-group': openyidaGlobalThemeRgba(rgb, 1) + ',rgba(0, 122, 255, 1),rgba(138, 92, 253, 1),rgba(30, 41, 128, 1),rgba(255, 107, 53, 1),rgba(0, 200, 255, 1)',
    '--color-brand1-10': openyidaGlobalThemeRgba(rgb, 0.3),
    '--color-brand1-9': openyidaGlobalThemeRgba(deep, 1)
  };
}

function openyidaGlobalThemeInjectIntoDocument(doc) {
  if (!doc || !doc.head || !doc.documentElement) {
    return;
  }
  var themeConfig = openyidaGlobalThemeBuildVariables(openyidaGlobalThemeResolveColor(doc));
...
export function openyidaInjectYidaGlobalTheme() {
  openyidaGlobalThemeGetWindowDocuments().forEach(openyidaGlobalThemeInjectIntoDocument);
}

export function ${YIDA_GLOBAL_THEME_DID_MOUNT}(event) {
  if (typeof ${previousName} === 'function') {
    ${previousName}.call(this, event);
  }
  return openyidaInjectYidaGlobalTheme();
}
`;
}

function ensureYidaGlobalThemeAction(schema) {
  ...
  const previousActionName = currentLifeCycle && currentLifeCycle.type === 'actionRef'
    ...
    : 'didMount';
  const generatedSource = buildYidaGlobalThemeActionSource(previousActionName);

  root.lifeCycles = root.lifeCycles || {};
  root.lifeCycles.componentDidMount = {
    name: YIDA_GLOBAL_THEME_DID_MOUNT,
    id: YIDA_GLOBAL_THEME_DID_MOUNT,
    params: {},
    type: 'actionRef',
  };
  actions.list = (actions.list || []).filter(function (item) {
    return item.id !== LEGACY_DIVIDER_THEME_DID_MOUNT;
  });
  ...
  const sourceWithoutLegacyBlock = removeGeneratedSourceBlockWithBounds(
    actions.module.source || '',
    LEGACY_DIVIDER_THEME_BLOCK_START,
    LEGACY_DIVIDER_THEME_BLOCK_END
  );
  actions.module.source = upsertGeneratedSourceBlockWithBounds(
    sourceWithoutLegacyBlock,
    generatedSource,
    YIDA_GLOBAL_THEME_BLOCK_START,
    YIDA_GLOBAL_THEME_BLOCK_END
  );
...
const ensureDividerThemeAction = ensureYidaGlobalThemeAction;
