...
function lintYidaSource(sourceCode, filePath) {
  ...
  lines.forEach((line, index) => {
    ...
    const formNewTabMatch = line.match(/\b(?:window\.open|(?:this|self)\.utils\.openPage)\s*\([^;\n]*(?:\/submission\/|\/formDetail\/|submitUrl|detailUrl|formOpenUrl)/i);
    const formOpenContext = lines.slice(Math.max(0, index - 2), index + 1).join(' ');
    if (formNewTabMatch && !isInCommentOrString(line, formNewTabMatch.index) && !/isMobile|utils\.isMobile/.test(formOpenContext)) {
      pushIssue(warnings, lineNumber, 'form-open-container', t('publish.lint_form_open_container'), disableMap);
    }
