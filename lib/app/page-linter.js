...
// 直连内部端点 searchFormDatas.json 的高频错误（看板/列表数据全 0 或分页失效的元凶）：
//  1. 路径写成 /query/form/searchFormDatas.json → 正确路径是 /v1/form/searchFormDatas.json。
//  2. 用 POST 把 formUuid 放进 body → 后端报「参数校验失败formUuid」，必须 GET + query。
//  3. 分页参数写成 pageNumber → 正确参数名是 currentPage。
//  4. 缺 appType/searchFieldJson/_csrf_token/credentials → 登录态或过滤契约不完整。
//  5. 只读 json.data → 浏览器直连的列表在 content.data，需要兜底解包。
...
function detectDirectFormEndpointMisuse(sourceCode, errors, warnings, disableMap) {
  ...
  const hasQueryParam = function hasQueryParam(name) {
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const pattern = new RegExp(
      '(?:[?&]' + escaped + '=|\\.set\\(\\s*[\'"]' + escaped + '[\'"]|[\'"]' + escaped + '[\'"]\\s*:|\\b' + escaped + '\\s*:)',
      'i'
    );
    return pattern.test(strippedSource);
  };
  ...
  while ((idx = sourceCode.indexOf(ENDPOINT, searchIndex)) >= 0) {
    ...
    if (/\/query\/form\/searchFormDatas\.json/.test(windowText)) {
      pushIssue(errors, line, 'searchformdata-http-path', t('publish.lint_searchformdata_http_path'), disableMap);
    }
  ...
  const missingParams = ['appType', 'searchFieldJson'].filter(name => !hasQueryParam(name));
  if (firstEndpointLine && missingParams.length) {
    pushIssue(warnings, firstEndpointLine, 'searchformdata-http-query-params', t('publish.lint_searchformdata_http_query_params', missingParams.join(', ')), disableMap);
  }
  if (firstEndpointLine && !hasQueryParam('_csrf_token')) {
    pushIssue(warnings, firstEndpointLine, 'searchformdata-http-csrf', t('publish.lint_searchformdata_http_csrf'), disableMap);
  }
  if (firstEndpointLine && !/\bcredentials\s*:\s*['"]include['"]/.test(strippedSource)) {
    pushIssue(warnings, firstEndpointLine, 'searchformdata-http-credentials', t('publish.lint_searchformdata_http_credentials'), disableMap);
  }
...
function isCanvasSourceFile(filePath) {
  return /\.canvas\.(jsx|tsx)$/i.test(String(filePath || ''));
}

function detectMissingCanvasYidaApiBridge(sourceCode, filePath, errors, disableMap) {
  if (!isCanvasSourceFile(filePath)) {
    return;
  }
  const strippedSource = stripCommentsOnly(sourceCode);
  const readsFormData = /searchFormDatas(?:\.json|\s*\()/.test(strippedSource)
    || /OPENYIDA_DATA_BINDING_JSON/.test(strippedSource)
    || /\bDATA_BINDING\b/.test(strippedSource);
  if (!readsFormData) {
    return;
  }
  if (/__OPENYIDA_YIDA_API__/.test(strippedSource)) {
    return;
  }
  pushIssue(errors, 1, 'canvas-yida-api-bridge-missing', t('publish.lint_canvas_yida_api_bridge_missing'), disableMap);
}
...
function lintYidaSource(sourceCode, filePath) {
  ...
  detectMissingCanvasYidaApiBridge(sourceCode, filePath, errors, disableMap);
  ...
  findBareCjkJsxTextIdentifiers(sourceCode, { parserOptions: PARSER_OPTIONS }).forEach((issue) => {
    pushIssue(errors, issue.line, 'jsx-text-identifier', t('publish.lint_jsx_text_identifier', issue.name), disableMap);
  });
  ...
  findDirectFormOpenIssues(sourceCode, { parserOptions: PARSER_OPTIONS }).forEach((issue) => {
    pushIssue(errors, issue.line, 'form-open-container', t('publish.lint_form_open_container'), disableMap);
  });
