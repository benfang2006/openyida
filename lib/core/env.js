...
function getInstalledTools() {
  const tools = [
    ...
    { dirName: '.qwenworkcn', displayName: 'QwenWork（千问办公）' },
    { dirName: '.qoderwork', displayName: 'QoderWork' },
    { dirName: '.qoder', displayName: 'Qoder / Qoder IDE' },
  ];
...
function detectEnvironment() {
  ...
  return { activeTool, activeToolName, activeProjectRoot, results };
}
function getTokenDiagnostics(projectRoot) {
  ...
  const tokenFile = getTokenFilePath({ projectRoot });
  const authProfilePointerFile = getAuthProfilePointerFilePath({ projectRoot });

  return {
    currentEnv: currentEnv.name,
    tokenFile,
    tokenFileFound: fs.existsSync(tokenFile),
    authProfilePointerFile,
    authProfilePointerFileFound: fs.existsSync(authProfilePointerFile),
    userAuthDir: resolveUserAuthDir(),
    userAuthStoreWritable: getUserAuthStoreWritable(),
  };
}
...
function detectLoginStatus(projectRoot) {
  ...
  return {
    ...
    corpName: status.corp_name || null,
    userId: status.user_id || null,
    baseUrl: status.base_url || null,
    authSource: status.auth_source || 'token',
    authStore: status.auth_store || null,
    authProfile: status.auth_profile || null,
    persistenceScope: status.persistence_scope || null,
    userAuthStoreWritable: status.user_auth_store_writable,
    ...
    diagnostics: {
      ...
      corpNameFound: !!status.corp_name,
...
function buildEnvironmentSnapshot() {
  ...
  const activeTool = environment.activeTool || null;
  ...
  return {
    ...
    active: {
      ...
      toolId: activeTool ? activeTool.tool : null,
      toolKey: environment.results.find((item) => item.isActive)?.dirName || null,
      runtime: activeTool ? activeTool.runtime : null,
      subtype: activeTool ? activeTool.subtype : null,
      workspaceRootSource: activeTool ? activeTool.workspaceRootSource : null,
      capabilities: activeTool ? activeTool.capabilities : null,
    ...
    login: {
      ...
      authStore: loginStatus.authStore || null,
      authProfile: loginStatus.authProfile || null,
      persistenceScope: loginStatus.persistenceScope || null,
      userAuthStoreWritable: loginStatus.userAuthStoreWritable,
      warning: loginStatus.warning || null,
      authMode: loginStatus.authMode || null,
      corpId: loginStatus.corpId,
      corpName: loginStatus.corpName,
      userId: loginStatus.userId,
      tokenFile: loginStatus.tokenFile,
