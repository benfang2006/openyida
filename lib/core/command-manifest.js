  ...
  return {
    ...
    core_workflows: {
      full_app_fast_build: {
        ...
        form_visual_guidance_policy: 'Default form page development loads yida-form-detail as lightweight form visual guidance, merges Divider semantic grouping into field JSON, and does not inject formDetail CSS unless the user or delivery mode requires detail-page styling after a real formUuid exists.',
        ...
        prd_visual_policy: 'Every PRD mode must include visual specifications. Fast_build writes a lightweight visual spec with themeProfile, themeColorSource, light/dark mode, navigation visual policy, page mood, layout/density, and differentiation notes; deep_design expands this into the full low-code PRD visual specification section.',
        ui_guidance_policy: 'Default full-app primary page creation includes yida-page-uiux lightweight UI guidance to decide page type, template route, visualProfile, and sample-avoidance checks. This is not deep_design and must not pull yida-app-uiux, theme, demo data, screenshots, or long PRD work by default.',
        default_nav_order_policy: 'After the primary page is successfully published in fast_build, immediately run a lightweight navigation order update. Default priority is portal/home/workbench entry > custom display pages > process forms > receipt/forms; use openyida publish ... --auto-nav-order or openyida nav-group auto-order <appType>.',
        ...
        completion_contract: 'Create app, core forms, primary page, publish it with lightweight auto navigation order, and return access URLs. If three or more resources/links are returned, present them in a Markdown table with type, purpose/name, URL, and status.',
      },
    },
  };
}
