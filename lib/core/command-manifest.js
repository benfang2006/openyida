...
const COMMAND_SIDE_EFFECTS = new Map([
  ...
  ...sideEffectEntries([
    ...
    'create-report',
    'externalize-form',
    'form-detail-style.apply',
    'form-detail-style.remove',
    'import',
    'integration.create',
    'integration.disable',
    'integration.enable',
...
const COMMAND_GROUPS = [
  ...
  },
  {
    ...
    commands: [
      ...
      command('generate-sample', ['generate-sample'], 'generate-sample <sample>', 'help.cmd_generate_page'),
      command('generate-page', ['generate-page'], 'generate-page <sample>', 'help.cmd_generate_page', {
        hidden: true,
      }),
      ...
      command('form-detail-style.apply', ['form-detail-style', 'apply'], 'form-detail-style apply <appType> <formUuid> [--css file|--preset clean-card] [--json]', 'help.cmd_form_detail_style', { output: 'json' }),
      command('form-detail-style.remove', ['form-detail-style', 'remove'], 'form-detail-style remove <appType> <formUuid> [--json]', 'help.cmd_form_detail_style', { output: 'json' }),
      command('form-detail-style.check', ['form-detail-style', 'check'], 'form-detail-style check <appType> <formUuid> [--json]', 'help.cmd_form_detail_style', { output: 'json' }),
    ],
  },
  {
    id: 'data',
  ...
  },
  {
    ...
    commands: [
      ...
      command('asset', ['asset'], 'asset <status|verify-url|resolve|generate> [options]', 'help.cmd_asset', {
        requiresLogin: false,
        output: 'text|json',
      }),
  ...
  return {
    ...
    core_workflows: {
      full_app_build: {
        ...
        form_visual_guidance_policy: 'Default form page development loads yida-form-detail as form visual guidance, merges Divider semantic grouping into field JSON, and injects formDetail CSS by default after a real formUuid exists. New and updated form schemas should include the openyida:theme action block and openyidaThemeDidMount lifecycle action; existing forms should be checked/applied with openyida form-detail-style apply unless a concrete blocker is reported.',
        ...
        product_design_policy: 'Full app creation first resolves resource context, then uses yida-design for requirement analysis and product design before creating or filling missing app/form/process/page resources. yida-design outputs prd/<project>/prd.md with app information, app configuration, data structures, page functions, business rules, interaction states, resource blueprint, resource creation order, page implementation delivery order, navigation order, and acceptance criteria; it outputs prd/<project>/design.md with theme tokens, visual DNA, layout density, rounded rules, scene recipes, component rules, state rules, and visual acceptance checks.',
        ui_guidance_policy: 'Default full-app primary page creation consumes yida-design prd.md for positioning, information architecture, page prototype, native form entry policy, material strategy, and business-specific checks; it consumes design.md for theme system, visual states, visualScaffold, surface material, rounded rules, density rules, components, and state styling. prd.md and design.md are the only design sources of truth. Page implementation may extract page-spec.json as a derived implementation handoff from prd.md + design.md; conflicts are resolved by regenerating the spec from prd.md/design.md. Demo data, screenshots, public sharing, data-source deep binding, and fine navigation grouping are optional after explicit user request or PRD acceptance criteria.',
        default_nav_order_policy: 'After the primary page is successfully published, immediately run a lightweight navigation order update. If the PRD names a navigation order, call openyida nav-group order <appType> <items...>. If the PRD only gives broad groups or is missing navigation order, use openyida publish ... --auto-nav-order or openyida nav-group auto-order <appType> as a fallback with portal/home/workbench entry > business handling > data management > business analytics > system configuration.',
        final_link_policy: 'Return exactly one primary user-facing link. For single-page create/update/publish intent, return the current page URL. For full app, form, process, permission, theme, navigation, or batch-resource work, return the app workbench URL: {base_url}/{appType}/workbench. Summarize other resources by type/name/ID/status only; do not output static asset, CDN build, locale JSON, admin, config, schema, or intermediate-file URLs unless explicitly requested.',
        ...
        completion_contract: 'Resolve context, write the yida-design PRD, create or reuse app, core forms/processes, and primary page in PRD resource order, publish it, apply PRD navigation order or lightweight fallback navigation order, and return access URLs. If three or more resources/links are returned, present them in a Markdown table with type, purpose/name, URL, and status.',
      },
    },
  };
}
