 ...
 *   EnvironmentChecker       - 环境诊断（Node/npm/可选浏览器登录/config/Skills/登录态/网络）
...
/**
 * 环境诊断检查器。
 * 检查 Node.js、npm、可选浏览器登录支持、config.json、Skills、登录态、网络连通性。
 */
class EnvironmentChecker {
  ...
  async check() {
    return [
      ...
      this.checkOptionalBrowserLogin(),
      this.checkConfig(),
  ...
  checkLoginStatus() {
    ...
    if (!status.ok) {
      if (isEnvAuthMode()) {
        return {
          id: 'env-login',
          label: '宜搭运行环境 token 注入登录态',
          passed: false,
          severity: Severity.WARNING,
          message: `运行环境 token 注入不可用（${status.failure_reason || status.status}），请回到运行环境注入 OPENYIDA_ACCESS_TOKEN 或 OPENYIDA_REFRESH_TOKEN`,
          fixType: null,
          fixCommand: null,
        };
      }
