...
module.exports = {
  ...
  help: {
    ...
    cmd_generate_page: '基于内置示例生成页面骨架',
    ...
    cmd_form_detail_style: '管理表单详情页样式',
    group_data: '数据 & 权限',
    ...
    cmd_asset: '检测素材能力 / 校验图片 URL / 解析回填素材',
...
`,
    ...
    compile_example: '示例: openyida compile pages/src/home.canvas.jsx',
    check_page_usage: '用法: openyida check-page <源文件路径> [--compat] [--json]',
    check_page_example: '示例: openyida check-page pages/src/home.canvas.jsx --json',
    generate_page_usage: '用法: openyida generate-sample <sample> --output pages/src/home.canvas.jsx [--spec .cache/openyida/page-specs/home.json] [--theme-scope page|app] [--compile]',
    generate_page_example: '示例: openyida generate-sample product-homepage --brand-name OpenKuma --brand-initials OK --theme-scope page --output pages/src/home.canvas.jsx --compile',
    ...
    login_unsupported_option: '已删除的旧登录参数不再支持: {0}。请使用 token/OAuth 登录；运行环境注入模式下只检查运行环境注入的 token。',
    ...
    first_run_footer1: '  支持的 AI 工具：Codex / Claude Code / Aone Copilot / Cursor / OpenCode / QwenWork',
  ...
  create_page: {
    ...
    example: '示例: openyida create-page "APP_XXX" "驾驶舱" --mode dashboard；显式隐藏导航: openyida create-page "APP_XXX" "全屏看板" --mode dashboard --hide-nav',
    app_id: '  应用 ID:  {0}',
    page_name: '  页面名称: {0}',
    step_create: '\n📄 Step 2: 创建自定义页面\n',
    ...
    dashboard_config_ok: '  ✅ 已按显式要求隐藏导航，并输出无左侧栏的 custom URL',
    dashboard_config_failed: '  ⚠️  隐藏导航配置失败: {0}',
    err_mode_invalid: '不支持的页面模式: {0}',
    mode_hint: '可用模式: default, dashboard；默认导航可见，只有传 --hide-nav 或 --render-nav false 才隐藏导航',
  ...
  copy: {
    ...
    no_ai_tool: '\n❌ 未检测到活跃的 AI 工具环境\n   支持的工具：悟空、Codex、OpenCode、Claude Code、Aone Copilot、Cursor、QwenWork（千问办公）、QoderWork、Qoder\n\n   当前检测结果：',
    no_active_tool: '\n❌ 未检测到活跃的 AI 工具环境',
    supported_tools: '   支持的工具：悟空、Codex、OpenCode、Claude Code、Aone Copilot、Cursor、QwenWork（千问办公）、QoderWork、Qoder',
    current_result: '\n   当前检测结果：',
    force_hint: '\n   如需强制复制到当前目录，请运行：\n   openyida copy --force',
  ...
  publish: {
    ...
    lint_form_open_container: '自定义页内打开表单提交/详情时，PC 端应使用 FormOpenContainer 抽屉 iframe，移动端才整页或新页打开，避免默认新开标签',
