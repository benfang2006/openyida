...
Usage:
  ...
  openyida data create form <appType> <formUuid> (--data-json <JSON>|--data-file .cache/openyida/data.json) --expect-form-name <name> --expect-form-type receipt [--dept-id ID] [--resolve-aliases]
  openyida data update form <appType> --inst-id <formInstId> --form-uuid <formUuid> (--data-json <JSON>|--data-file .cache/openyida/data.json) --expect-form-name <name> --expect-form-type receipt [--use-latest-version y] [--resolve-aliases]
  ...
  openyida data create process <appType> <formUuid> --process-code <processCode> (--data-json <JSON>|--data-file .cache/openyida/data.json) --expect-form-name <name> --expect-form-type process [--dept-id ID] [--resolve-aliases]
  openyida data update process <appType> --process-inst-id <processInstanceId> --form-uuid <formUuid> (--data-json <JSON>|--data-file .cache/openyida/data.json) --expect-form-name <name> --expect-form-type process [--resolve-aliases]
  openyida data query operation-records <appType> --process-inst-id <processInstanceId>
  openyida data execute task <appType> --task-id <taskId> --process-inst-id <processInstanceId> --form-uuid <formUuid> --expect-form-name <name> --expect-form-type process --out-result <AGREE|DISAGREE> --remark <text> [--data-json JSON|--data-file .cache/openyida/data.json] [--resolve-aliases] [--no-execute-expressions y]
...
const MUTATION_DATA_COMMANDS = new Set([
  'create:form',
  'update:form',
  'delete:form',
  'create:process',
...
function validateDataCommandBeforeAuth(action, resource, positionals, options) {
  ...
  if (MUTATION_DATA_COMMANDS.has(`${action}:${resource}`)) {
    requireOption(options, 'expect_form_name');
    requireOption(options, 'expect_form_type');
    const expectedType = resource === 'form' ? 'receipt' : 'process';
    if (String(options.expect_form_type).trim().toLowerCase() !== expectedType) {
      dataError(
        'DATA_TARGET_EXPECTATION_INVALID',
        t('query_data.target_expectation_invalid', expectedType),
        {
          expectedFormType: expectedType,
          providedFormType: options.expect_form_type,
          retryable: false,
          retrySafe: true,
          sideEffectState: 'none',
          nextStep: 'resolve_target_identity',
        }
      );
    }
    if ((action === 'update' || resource === 'task') && !options.form_uuid) {
      parseError('--form-uuid 是写前目标身份校验的必填参数');
    }
  }
...
const ASSOCIATION_FORM_FIELD_PREFIX = 'associationFormField_';
const ASSOCIATION_FORM_DERIVED_ID_RE = /^associationFormField_.+_id$/;
const ASSOCIATION_FORM_REQUIRED_KEYS = [
  'appType',
  'formUuid',
  'formType',
  'instanceId',
...
function isAssociationFormField(fieldId, aliasContext) {
  const fieldInfo = aliasContext && aliasContext.fieldInfoById[fieldId];
  return fieldId.startsWith(ASSOCIATION_FORM_FIELD_PREFIX)
    || (fieldInfo && fieldInfo.componentName === 'AssociationFormField');
}

function throwAssociationFormValueInvalid(fieldId, index, reason, missingFields = []) {
  dataError(
    'DATA_ASSOCIATION_FORM_VALUE_INVALID',
    t('query_data.association_value_invalid', fieldId),
    {
      fieldId,
      index,
      reason,
      ...
      retrySafe: true,
      sideEffectState: 'none',
      nextStep: 'provide_complete_association_form_value',
    }
  );
}

function normalizeAssociationFormFields(data, aliasContext) {
  ...
  Object.entries(data).forEach(([fieldId, value]) => {
    if (ASSOCIATION_FORM_DERIVED_ID_RE.test(fieldId)) {
      dataError(
        'DATA_ASSOCIATION_FORM_DERIVED_FIELD_READ_ONLY',
        t('query_data.association_derived_field_read_only', fieldId),
        {
          fieldId,
          retryable: false,
          retrySafe: true,
          sideEffectState: 'none',
          nextStep: 'write_the_source_association_form_field',
        }
      );
    }

    if (!isAssociationFormField(fieldId, aliasContext)) {
      normalized[fieldId] = value;
      return;
    }
    if (!Array.isArray(value)) {
      throwAssociationFormValueInvalid(fieldId, null, 'value_must_be_array');
    }

    normalized[fieldId] = value.map((item, index) => {
      if (!isPlainObject(item)) {
        throwAssociationFormValueInvalid(fieldId, index, 'item_must_be_object');
      }
      const missingFields = ASSOCIATION_FORM_REQUIRED_KEYS.filter((key) => (
        typeof item[key] !== 'string' || item[key].trim() === ''
      ));
      if (missingFields.length > 0) {
        throwAssociationFormValueInvalid(fieldId, index, 'required_fields_missing', missingFields);
      }
      if (item.subTitle !== undefined && typeof item.subTitle !== 'string') {
        throwAssociationFormValueInvalid(fieldId, index, 'subtitle_must_be_string');
      }
      return {
        ...item,
...
function requireKnownDataFieldRef(value, aliasContext) {
  ...
  dataError(
    'DATA_FIELD_REFERENCE_UNKNOWN',
    t('query_data.field_reference_unknown', value),
    {
      fieldRef: value,
      retryable: false,
      retrySafe: true,
...
function translateFormDataObject(data, aliasContext) {
  ...
  Object.entries(data).forEach(([key, value]) => {
    const fieldId = requireKnownDataFieldRef(key, aliasContext);
    const fieldInfo = aliasContext && aliasContext.fieldInfoById[fieldId];
...
function translateJsonWithAliases(jsonValue, aliasContext, translator) {
  ...
  return JSON.stringify(translator(parsed, aliasContext));
}

function prepareFormMutationJson(jsonValue, aliasContext) {
  let data;
  try {
    data = JSON.parse(jsonValue);
  } catch (err) {
    parseError(`--data-json 只能处理合法 JSON：${err.message}`);
  }
  const translated = aliasContext
    ? translateFormDataObject(data, aliasContext)
    : data;
  return JSON.stringify(normalizeAssociationFormFields(translated, aliasContext));
}
...
function normalizeTargetFormType(value) {
  const normalized = String(value || '').trim().toLowerCase();
  if (normalized === 'form' || normalized === 'normal-form') {return 'receipt';}
  if (normalized === 'process-form' || normalized === 'workflow') {return 'process';}
  return normalized;
}

async function requireExpectedFormTarget(session, appType, formUuid, options) {
  const expected = {
    appType,
    formUuid,
    formName: String(options.expect_form_name || '').trim(),
    formType: normalizeTargetFormType(options.expect_form_type),
  };
  let forms;
  try {
    forms = await fetchFormPageList(appType, session);
  } catch (error) {
    dataError(
      'DATA_TARGET_IDENTITY_UNVERIFIED',
      t('query_data.target_identity_unverified', formUuid),
      {
        expected,
        causeCode: error && error.code || 'FORM_LIST_READ_FAILED',
        retryable: false,
        retrySafe: true,
        sideEffectState: 'none',
        nextStep: 'resolve_target_identity',
  ...
  const matches = (Array.isArray(forms) ? forms : [])
    .filter(form => form && String(form.formUuid || '') === String(formUuid));
  const nameCandidates = (Array.isArray(forms) ? forms : [])
    .filter(form => form && String(form.formName || '').trim() === expected.formName)
    .slice(0, 10)
    .map(form => ({
      formUuid: form.formUuid,
      formName: String(form.formName || '').trim(),
      formType: normalizeTargetFormType(form.formType),
    }));
  ...
  const observed = target
    ? {
      ...
      formName: String(target.formName || '').trim(),
      formType: normalizeTargetFormType(target.formType),
    }
    : null;

  if (!target || observed.formName !== expected.formName || observed.formType !== expected.formType) {
    dataError(
      'DATA_TARGET_IDENTITY_MISMATCH',
      t('query_data.target_identity_mismatch', formUuid),
      {
        expected,
        observed,
        ...
        retrySafe: true,
        sideEffectState: 'none',
        nextStep: 'stop_and_resolve_target_identity',
      }
    );
  }
  return observed;
...
function requireInstanceTarget(record, expected, instanceId, idKeys) {
  const observedInstanceId = normalizeResourceId(
    idKeys.map(key => record && record[key]).find(Boolean)
  );
  const observedFormUuid = normalizeResourceId(record && (record.formUuid || record.modelUuid));
  if (!record || observedInstanceId !== normalizeResourceId(instanceId)
    || observedFormUuid !== normalizeResourceId(expected.formUuid)) {
    dataError(
      'DATA_INSTANCE_TARGET_MISMATCH',
      t('query_data.instance_target_mismatch', instanceId, expected.formUuid),
      {
        expected: { ...expected, instanceId: normalizeResourceId(instanceId) },
        observed: { instanceId: observedInstanceId, formUuid: observedFormUuid },
        retryable: false,
        retrySafe: true,
        sideEffectState: 'none',
        nextStep: 'stop_and_resolve_target_identity',
...
async function readProcessInstance(session, appType, processInstanceId) {
  const result = await sendGet(
    session,
    appType,
    `/dingtalk/web/${appType}/v1/process/getInstanceById.json`,
    { processInstanceId }
  );
  const record = getFormRecordFromResult(result);
  if (!record || typeof record !== 'object' || Array.isArray(record)) {
    dataError(
      'DATA_INSTANCE_TARGET_UNVERIFIED',
      t('query_data.instance_target_unverified', processInstanceId),
      {
        retryable: false,
        retrySafe: true,
        sideEffectState: 'none',
...
async function createForm(positionals, options, session) {
  ...
  const [appType, formUuid] = positionals;
  const target = await requireExpectedFormTarget(session, appType, formUuid, options);
  const aliasContext = await resolveAliasContextIfNeeded(session, appType, formUuid, options, dataJson);
  const params = {
    appType,
    formUuid,
    formDataJson: prepareFormMutationJson(dataJson, aliasContext),
  };
  ...
  if (formMode.mode === 'process') {
    dataError(
      ...
      {
        expected: target,
        observed: { ...target, formType: 'process', processCode: formMode.processCode },
        retryable: false,
        retrySafe: true,
        sideEffectState: 'none',
...
async function updateForm(positionals, options, session) {
  ...
  const dataJson = requireJsonOption(options, 'data_json', 'data_file', '数据');
  const [appType] = positionals;
  const target = await requireExpectedFormTarget(session, appType, options.form_uuid, options);
  const preflight = await sendGet(session, appType, `/dingtalk/web/${appType}/v1/form/getFormDataById.json`, {
    formInstId: options.inst_id,
  });
  const record = getFormRecordFromResult(preflight);
  requireInstanceTarget(record, target, options.inst_id, ['formInstId']);
  const aliasContext = await resolveAliasContextIfNeeded(session, appType, options.form_uuid, options, dataJson);
  const params = {
    formInstId: options.inst_id,
    updateFormDataJson: prepareFormMutationJson(dataJson, aliasContext),
  };
  if (options.use_latest_version) {params.useLatestVersion = options.use_latest_version;}
  printResult(await sendPostOnce(session, appType, `/dingtalk/web/${appType}/v1/form/updateFormData.json`, params));
}
...
async function deleteForm(positionals, options, session) {
  ...
  await requireExpectedFormTarget(session, appType, formUuid, options);
...
async function createProcess(positionals, options, session) {
  ...
  const target = await requireExpectedFormTarget(session, appType, formUuid, options);
  const formMode = await requireVerifiedFormMode(session, appType, formUuid);
  if (formMode.mode !== 'process' || formMode.processCode !== options.process_code) {
    dataError(
      'DATA_TARGET_IDENTITY_MISMATCH',
      t('query_data.target_identity_mismatch', formUuid),
      {
        expected: { ...target, processCode: options.process_code },
        observed: { ...target, formType: formMode.mode, processCode: formMode.processCode || null },
        retryable: false,
        retrySafe: true,
        sideEffectState: 'none',
        nextStep: 'stop_and_resolve_target_identity',
  ...
  if (options.dept_id) {params.deptId = options.dept_id;}
  printCreateResult(
    await sendPostOnce(session, appType, `/dingtalk/web/${appType}/v1/process/startInstance.json`, params),
    'processInstance',
    'processInstanceId'
  );
}
...
async function updateProcess(positionals, options, session) {
  ...
  const [appType] = positionals;
  const target = await requireExpectedFormTarget(session, appType, options.form_uuid, options);
  const record = await readProcessInstance(session, appType, options.process_inst_id);
  requireInstanceTarget(
    record,
    target,
    options.process_inst_id,
    ['processInstanceId', 'processInstId', 'instanceId']
  );
  const aliasContext = await resolveAliasContextIfNeeded(session, appType, options.form_uuid, options, dataJson);
  printResult(await sendPostOnce(session, appType, `/dingtalk/web/${appType}/v1/process/updateInstance.json`, {
    processInstanceId: options.process_inst_id,
    updateFormDataJson: prepareFormMutationJson(dataJson, aliasContext),
  }));
}
...
async function executeTask(positionals, options, session) {
  ...
  const aliasContext = await resolveAliasContextIfNeeded(session, appType, options.form_uuid, options, dataJson);
  if (dataJson) {params.formDataJson = prepareFormMutationJson(dataJson, aliasContext);}
  if (options.no_execute_expressions) {params.noExecuteExpressions = options.no_execute_expressions;}
  printResult(await sendPostOnce(session, appType, `/dingtalk/web/${appType}/v1/task/executeTask.json`, params));
}
