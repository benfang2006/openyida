...
function buildAppWorkbenchUrl(payload = {}) {
  const appType = payload.appType ? String(payload.appType).trim() : '';
  if (!appType) {
    return '';
  }

  const sourceUrl = safeUrl(payload.url || payload.workbenchUrl || payload.baseUrl);
  if (!sourceUrl) {
    return '';
  }
  ...
  try {
    ...
    return `${url.origin}/${encodeURIComponent(appType)}/workbench`;
  } catch {
    return '';
  }
}
...
function sanitizeCreateAppResult(result = {}, params = {}) {
  ...
  const appType = payload.appType ? String(payload.appType).slice(0, 120) : '';
  const workbenchUrl = buildAppWorkbenchUrl({ ...payload, appType });
  const url = workbenchUrl || safeUrl(payload.workbenchUrl || payload.url);
  return {
    success: payload.success !== false && (payload.success === true || !!appType || !!url),
    appName: String(payload.appName || params.appName || '').slice(0, 80),
    appType,
    url,
    workbenchUrl: url,
    baseUrlOrigin: originFromUrl(url || payload.url || payload.baseUrl),
