...
function hasLocalBrowserLoginCapability(options = {}) {
  const env = options.env || process.env;
  const platform = options.platform || process.platform;
  const playwrightFallback = options.playwrightFallback !== false;
  const { hasDesktopEnvironment } = require('../core/utils');

  if (!hasDesktopEnvironment(env, platform)) {
    return false;
  }

  if (env.OPENYIDA_DISABLE_CDP_LOGIN !== '1') {
    try {
      const { findBrowserExecutable } = require('./cdp-browser-login');
      if (findBrowserExecutable()) {
        return true;
      }
    } catch {
      // ignore and try Playwright fallback below
    }
  }

  if (!playwrightFallback || env.OPENYIDA_DISABLE_PLAYWRIGHT_LOGIN === '1') {
    return false;
  }

  return !!getPlaywrightPath();
}
...
module.exports = {
  ...
  interactiveLogin,
  hasLocalBrowserLoginCapability,
  saveCookieCache,
  logout,
};
