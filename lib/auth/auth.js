...
const { logout, checkLoginOnly, interactiveLogin, hasLocalBrowserLoginCapability } = require('./login');
...
async function authLogin(options = {}) {
  ...
  } else if (type === 'qrcode' && !forceTerminalQr && hasLocalBrowserLoginCapability({ playwrightFallback: true })) {
    result = interactiveLogin({ playwrightFallback: true }) ||
...
async function loginInAgentConversation(options = {}) {
  ...
  const browserResult = hasLocalBrowserLoginCapability({ playwrightFallback })
