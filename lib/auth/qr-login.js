...
function buildBrowserLoginCommand(envName, options = {}) {
  const envArg = envName ? ` --env ${shellQuote(envName, options)}` : '';
  return `openyida login --browser${envArg}`;
}
...
function buildAgentQrResponseMarkdown(result) {
  ...
  if (result.browser_login_command) {
    lines.push(t('qr_login.browser_alternative', result.browser_login_command).trim(), '');
  }
...
function buildNeedQrScanResult({ qrUrl, qrImageFile, qrImageOpened, sessionFile, targetCorpId, envName, platform }) {
  ...
  const result = {
    ...
    browser_login_command: buildBrowserLoginCommand(envName, { platform }),
...
async function qrLogin(options = {}) {
  ...
  process.stderr.write('\n');
  qLabel('URL', qrUrl);
  qInfo(t('qr_login.browser_alternative', buildBrowserLoginCommand(currentEnvName)));
  process.stderr.write('\n');
