 ...
 *
 * 用法（更新已有权限组）：
 *   openyida save-permission <appType> <formUuid> [--package-uuid <packageUuid>] --data-permission <json>
...
function parseArgs(args) {
  if (args.length < 2) {
    throw new CliError([
      '用法: openyida save-permission <appType> <formUuid> [--package-uuid <packageUuid>] [--create --name <名称>] [--data-permission <json>] [--action-permission <json>] [--field-permission <json>] [--members <userIds>] [--all-members] [--matrix <json>]',
  ...
  for (let index = 2; index < args.length; index++) {
    if (args[index] === '--create') {
      ...
      packageUuid = args[index + 1];
      index++;
    } else {
      throw new CliError(`不支持的参数: ${args[index]}`, {
        code: 'SAVE_PERMISSION_INVALID_ARGUMENTS',
      });
    }
  }
  ...
  if (createMode && packageUuid) {
    throw new CliError(t('save_permission.package_uuid_create_conflict'), {
      code: 'SAVE_PERMISSION_INVALID_ARGUMENTS',
    });
  }
...
function packageSnapshot(permitPackage) {
  return VERIFIED_PACKAGE_FIELDS.reduce((snapshot, field) => {
    snapshot[field] = normalizePackageField(permitPackage && permitPackage[field]);
    return snapshot;
  }, {});
}
...
function verifyPackageDimensions(before, expectedPackage, actualPackage, targetDimensions) {
  const targetSet = new Set(targetDimensions);
  const expectedSnapshot = packageSnapshot(expectedPackage);
  const actualSnapshot = packageSnapshot(actualPackage);
  const beforeSnapshot = before ? packageSnapshot(before) : null;
  const mismatches = [];

  for (const dimension of VERIFIED_PACKAGE_FIELDS) {
    const expected = expectedSnapshot[dimension];
    const actual = actualSnapshot[dimension];
    if (!valuesEqual(expected, actual)) {
      mismatches.push({
        dimension,
        kind: targetSet.has(dimension) || !before ? 'target' : 'preserved',
        expected,
        actual,
        before: beforeSnapshot && beforeSnapshot[dimension],
      });
    }
  }
  return { expected: expectedSnapshot, actual: actualSnapshot, mismatches };
}

function extractCreatedPackageUuid(result) {
  if (!result) {
    return null;
  }
  ...
  if (result.content && typeof result.content === 'object'
    && typeof result.content.packageUuid === 'string' && result.content.packageUuid.trim()) {
    return result.content.packageUuid.trim();
  }
  if (typeof result.packageUuid === 'string' && result.packageUuid.trim()) {
    return result.packageUuid.trim();
  }
  return null;
}

function packageReadableName(permitPackage) {
  const name = normalizePackageField(permitPackage && permitPackage.packageName);
  if (typeof name === 'string') {
    return name;
  }
  return name && (name.zh_CN || name.en_US) || '';
}

function throwVerificationError(code, packageUuid, details) {
  const status = code === 'SAVE_PERMISSION_VERIFY_FAILED' ? 'failed' : 'unknown';
  throw new CliError(
    t(
      status === 'failed' ? 'save_permission.verify_failed' : 'save_permission.verify_unknown',
      packageUuid || t('save_permission.missing_package_uuid')
    ),
    {
      code,
      details: { packageUuid: packageUuid || null, status, ...details },
    }
  );
}
...
async function run(args) {
  ...
  let beforeQuery;
  try {
    beforeQuery = await fetchAllPermitPackages(appType, formUuid, authRef);
  } catch (error) {
    throw new CliError(t('save_permission.before_unknown', error.message), {
      code: error && error.code === 'NEED_LOGIN' ? 'NEED_LOGIN' : 'SAVE_PERMISSION_BEFORE_UNKNOWN',
      details: { causeCode: error && error.code || null },
    });
  }
  const beforePackages = beforeQuery.packages;
  ...
  if (createMode) {
    ...
    if (createResult && createResult.success) {
      const responsePackageUuid = extractCreatedPackageUuid(createResult);
      let afterPackages;
      try {
        afterPackages = (await fetchAllPermitPackages(appType, formUuid, authRef)).packages;
      } catch (error) {
        throwVerificationError('SAVE_PERMISSION_VERIFY_UNKNOWN', responsePackageUuid, {
          reason: 'readback_failed',
          causeCode: error && error.code || null,
        });
      }

      let identitySource = 'response';
      let createdPackage = null;
      if (responsePackageUuid) {
        createdPackage = afterPackages.find(item => item && item.packageUuid === responsePackageUuid) || null;
        if (!createdPackage) {
          throwVerificationError('SAVE_PERMISSION_VERIFY_UNKNOWN', responsePackageUuid, {
            reason: 'response_uuid_not_found',
          });
        }
      } else {
        identitySource = 'owned-readback';
        const beforeUuids = new Set(beforePackages.map(item => item && item.packageUuid).filter(Boolean));
        const ownedCandidates = afterPackages.filter((item) => {
          if (!item || !item.packageUuid || beforeUuids.has(item.packageUuid)) {
            return false;
          }
          if (packageReadableName(item) !== groupName) {
            return false;
          }
          return verifyPackageDimensions(null, newPkg, item, VERIFIED_PACKAGE_FIELDS).mismatches.length === 0;
        });
        if (ownedCandidates.length !== 1) {
          throwVerificationError('SAVE_PERMISSION_VERIFY_UNKNOWN', null, {
            reason: ownedCandidates.length === 0 ? 'owned_readback_not_found' : 'owned_readback_ambiguous',
            candidateUuids: ownedCandidates.map(item => item.packageUuid),
          });
        }
        createdPackage = ownedCandidates[0];
      }

      const verification = verifyPackageDimensions(
        null,
        newPkg,
        createdPackage,
        VERIFIED_PACKAGE_FIELDS
      );
      if (verification.mismatches.length > 0) {
        throwVerificationError('SAVE_PERMISSION_VERIFY_FAILED', createdPackage.packageUuid, verification);
      }
      const newPackageUuid = createdPackage.packageUuid;
      warn('  ✅ 权限组新增成功！');
      warn(SEP);
      ...
      const output = {
        ...
        verification: {
          status: 'verified',
          identitySource,
          targetDimensions: VERIFIED_PACKAGE_FIELDS,
        },
        summary: {
          name: groupName,
  ...
  const targetPackage = packageUuid
    ? packages.find(item => item && item.packageUuid === packageUuid)
    : assertUniqueTarget(packages, targetRole);
  if (!targetPackage) {
    throw new CliError(t('save_permission.package_uuid_not_found', packageUuid), {
      code: 'SAVE_PERMISSION_PACKAGE_UUID_NOT_FOUND',
      details: { packageUuid },
    });
  }
  ...
  if (allSuccess) {
    ...
    try {
      ...
      throwVerificationError('SAVE_PERMISSION_VERIFY_UNKNOWN', pkg.packageUuid, {
        reason: 'readback_failed',
        causeCode: error && error.code || null,
      });
    }
    const readbackPackage = afterPackages.find(item => item && item.packageUuid === pkg.packageUuid);
    if (!readbackPackage) {
      throwVerificationError('SAVE_PERMISSION_VERIFY_UNKNOWN', pkg.packageUuid, {
    ...
    if (dataPermission) {targetDimensions.push('dataPermit');}
    if (actionPermission) {targetDimensions.push('operatePermit');}
    if (normalizedFieldPermission) {targetDimensions.push('fieldPermit');}
    if (memberDimensionChanged) {targetDimensions.push('roleData');}
    const verification = verifyPackageDimensions(
      targetPackage,
      updatedPkg,
      readbackPackage,
    ...
    if (verification.mismatches.length > 0) {
      throwVerificationError('SAVE_PERMISSION_VERIFY_FAILED', pkg.packageUuid, verification);
    }
    ...
    const output = {
      ...
      verification: {
        status: 'verified',
        identitySource: 'packageUuid',
        targetDimensions,
        preservedDimensions: VERIFIED_PACKAGE_FIELDS.filter(field => !targetDimensions.includes(field)),
      },
      summary,
      message: '权限配置已保存',
    };
...
module.exports = {
  ...
  savePermitPackage,
  packageSnapshot,
  verifyPackageDimensions,
  extractCreatedPackageUuid,
};
