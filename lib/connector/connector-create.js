...
const { HTTP_CONNECTOR_MODE } = require('./contract');
...
async function run(args) {
  ...
  if (options.isUpdateMode) {
    ...
    await saveConnector({
      ...
      connectorMode: detail.connectorMode || String(HTTP_CONNECTOR_MODE),
  ...
  const saveResult = await saveConnector({
    ...
    faasTemplateId: '0',
    securitySchemes,
    connectorMode: String(HTTP_CONNECTOR_MODE),
    connectorName,
    category: 'http',
  }, authRef);
  ...
  const result = {
    ...
    connectorName,
    displayName: options.name,
    readbackVerified: saveResult.readbackVerified === true,
  };
  if (options.json) {
