...
async function listConnectors(options, authRef) {
  ...
  let apiPath = `/query/newconnector/listConnector.json?_api=ConnectorFactory.getConnectorList&pageSize=${pageSize}&currentPage=${currentPage}&connectorMode=${HTTP_CONNECTOR_MODE}`;
...
async function listAllConnectors(options, authRef) {
  ...
  const error = new Error(t('connector_api.connector_pagination_limit'));
...
async function saveConnector(params, authRef) {
  ...
  const bodyParams = {
    ...
    connectorMode: params.connectorMode || String(HTTP_CONNECTOR_MODE),
  ...
  if (!connectorId && !responseConnectorName) {
    const error = new Error(t('connector_api.write_identity_missing'));
...
async function listAllConnections(connectorName, authRef) {
  ...
  const error = new Error(t('connector_api.connection_pagination_limit'));
...
async function createConnection(params, authRef) {
  ...
  if (before.some(connection => connection.connectionName === params.connectionName)) {
    const error = new Error(t('connector_api.connection_name_exists'));
  ...
  if (matches.length !== 1) {
    const error = new Error(t('connector_api.connection_readback_mismatch'));
...
async function testConnector(params, authRef) {
  ...
  return canonicalizeConnectorTestResponse(result);
}
