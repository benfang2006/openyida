...
6. **收敛 MVP 边界**：明确 V1 必做、V1 不做、V2 候选；会议没有明确时写入待确认事项
7. **提炼交互质量**：把“好用”“便捷”“智能”等模糊要求拆成可实现、可验收的交互规则
8. **识别运营视角**：除终端用户外，补齐管理员、审批人、调度员、数据查看者等后台运营角色
...
| AssociationFormField | 稳定关联其他表单记录（需在待确认事项中说明关联主字段） |
| SerialNumberField | 流水号/业务编号 |

### 质量标准
- 每个功能模块必须包含：功能描述、目标用户、交互设计、业务规则、字段设计表、验收标准
- 字段设计表必须包含：字段名、宜搭字段类型、必填、选项/格式、说明
- 必须明确用户角色与权限：谁使用前台工作台，谁维护后台表，谁审批/调度/查看报表
- 必须明确数据关联与约束：唯一字段、表间关系、状态字段、冲突校验、并发/重复提交风险
- 流程设计必须明确：发起人、审批节点、状态流转、通过/拒绝/取消后的动作
- 交互验收必须可验证：避免只写“体验好”，要写成“选择时间后即时提示冲突”“30 秒内完成预约”等标准
- OpenYida 落地约束必须明确：PRD 只写业务语义，不写 formUuid/fieldId；技术 ID 写入 .cache/<项目名>-schema.json
...
\`\`\`json
{
  ...
  "users": [{"role": "角色名", "scenario": "典型场景", "entry": "可见入口", "permissions": ["可执行操作"]}],
  "journeys": [{"name": "旅程名", "steps": ["步骤1"], "successCriteria": "成功标准"}],
  "features": [
    {
      "moduleName": "模块名",
      "description": "功能描述",
      ...
      "states": ["状态与流转"],
      "fields": [
        {"name": "字段名", "type": "宜搭字段类型", "required": true, "optionsOrFormat": "选项/格式", "description": "说明"}
      ],
      "acceptanceCriteria": ["验收标准"]
    }
  ],
  "processes": [{"name": "流程名", "steps": "流程描述"}],
  "statusMachine": [{"object": "对象", "status": "状态", "allowedAction": "允许操作", "nextStatus": "下一状态", "operatorRole": "操作角色"}],
  "constraints": [{"name": "约束", "rule": "规则", "handling": "处理方式"}],
  "reports": [{"name": "报表名", "source": "数据来源", "chartType": "图表类型", "description": "说明"}],
  "nonFunctional": ["非功能需求1"],
  "openYidaNotes": ["OpenYida落地约束或风险"],
...
- ## MVP 范围与版本边界
- ## 核心用户旅程
- ## 功能需求（每个模块含功能描述、目标用户、交互设计、业务规则、字段设计表、验收标准）
- ## 页面与表单配置
- ## 流程设计
- ## 流程与状态机
- ## 数据关联与约束
...
function createYidaAICaller(maxTokens = 8000) {
  return async function callYidaAI(prompt) {
    const csrfToken = globalThis.window && globalThis.window.g_config && globalThis.window.g_config._csrf_token;
