...
async function preflightReportSourceFields(authRef, appType, charts, filters) {
  ...
  for (const [cubeCode, fieldCodes] of configured.entries()) {
    ...
    try {
      ...
      throw new CliError(`无法读取报表数据源 ${formUuid} 的字段 Schema：${error.message}`, {
    ...
    if (missing.length > 0) {
      throw new CliError(`报表数据源 ${formUuid} 不存在配置字段：${missing.join(', ')}`, {
...
async function main(args) {
  ...
  try {
    ...
    warn(
      '组织上下文:',
      corpId ? '已确认（敏感标识已隐藏）' : '未获取到，图表数据源需手动配置',
    );
    ...
    const verification = assertReportSchemaReadback(expectedSchema, normalizeReportSchemaContent(readbackResult));
