...
const { validateChartConfigDetailed } = require('./validation');
...
function validateChartConfig(chart, chartIndex) {
  const issues = validateChartConfigDetailed(chart, chartIndex);
  issues.forEach((entry) => warn(`⚠️  ${entry.path}: ${entry.message}`));
...
function requireChartCapability(chartType) {
...
function buildChartNode(chart, cubeTenantId, options = {}) {
  const capability = requireChartCapability(chart && chart.type);
  const componentName = capability.componentName;
  const fieldId = genFieldId(componentName);
  const nodeId = genNodeId();
  const chartTitle = chart.title || options.fallbackTitle || `${componentName}_${randomId().slice(0, 4)}`;
  const w = chart.w !== null && chart.w !== undefined ? chart.w : capability.layout.w;
  const h = chart.h !== null && chart.h !== undefined ? chart.h : capability.layout.h;
  const dataSetModelMap = buildDataSetModelMap(chart, cubeTenantId);
  const userConfig = buildUserConfigWithFields(capability.type, dataSetModelMap);
  ...
  return {
    ...
    node: {
      ...
      props: {
        ...
        __style__: {},
        mockData: buildMockData(capability.type),
        dataSetModelMap,
        userConfig,
        settings: getChartSettings(capability.type),
        titleTip: false,
        hasFullscreen: false,
        copyAsImg: false,
        height: null,
        isHeightAuto: capability.isHeightAuto,
        ...(capability.showFieldSelectIcon ? { showFieldSelectIcon: true } : {}),
...
function buildReportComponentMapEntry(componentName) {
  const entry = {
    package: '@/components/vc-yida-report',
    version: '1.0.6',
    componentName,
  };
  if (componentName === 'YoushuSimpleIndicatorCard') {
    entry.userConfig = buildUserConfigWithFields('indicator');
  }
  return entry;
}
...
function buildReportSchema(reportTitle, charts, reportId, cubeTenantId) {
  ...
  charts.forEach((chart) => {
    usedComponentNames.add(requireChartCapability(chart.type).componentName);
  });

  const componentsMap = Array.from(usedComponentNames).map(buildReportComponentMapEntry);

  const builtCharts = charts.map((chart, index) => buildChartNode(chart, cubeTenantId, {
    fallbackTitle: `${requireChartCapability(chart.type).componentName}_${index + 1}`,
  }));
  const chartChildren = builtCharts.map(built => built.node);
  const layoutItems = packReportLayout(builtCharts.map(built => ({
    i: built.fieldId,
    w: built.w,
    h: built.h,
  })));
