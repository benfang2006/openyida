...
function assertDimension(item, index) {
  if (!item || typeof item !== 'object') {
    throw layoutError('REPORT_LAYOUT_INVALID', `layout[${index}] must be an object`, { index });
  }
  if (!Number.isInteger(item.w) || item.w <= 0 || item.w > GRID_COLUMNS) {
    throw layoutError('REPORT_LAYOUT_INVALID', `layout[${index}].w must be a positive integer no greater than ${GRID_COLUMNS}`, { index, field: 'w', value: item.w });
  }
  if (!Number.isInteger(item.h) || item.h <= 0) {
    throw layoutError('REPORT_LAYOUT_INVALID', `layout[${index}].h must be a positive integer`, { index, field: 'h', value: item.h });
  }
  if (typeof item.i !== 'string' || !item.i) {
    throw layoutError('REPORT_LAYOUT_INVALID', `layout[${index}].i must be a non-empty string`, { index, field: 'i' });
  }
}
...
function assertReportLayout(layout) {
  if (!Array.isArray(layout)) {
    throw layoutError('REPORT_LAYOUT_INVALID', 'layout must be an array');
  }
  const ids = new Set();
  layout.forEach((item, index) => {
    assertDimension(item, index);
    if (!Number.isInteger(item.x) || item.x < 0 || item.x + item.w > GRID_COLUMNS) {
      throw layoutError('REPORT_LAYOUT_INVALID', `layout[${index}].x is outside the ${GRID_COLUMNS}-column grid`, { index, field: 'x', value: item.x });
    }
    if (!Number.isInteger(item.y) || item.y < 0) {
      throw layoutError('REPORT_LAYOUT_INVALID', `layout[${index}].y must be a non-negative integer`, { index, field: 'y', value: item.y });
    }
    if (ids.has(item.i)) {
      throw layoutError('REPORT_LAYOUT_INVALID', `layout id is duplicated: ${item.i}`, { index, field: 'i', value: item.i });
    }
    ids.add(item.i);
  });
  ...
  for (let leftIndex = 0; leftIndex < layout.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < layout.length; rightIndex += 1) {
      if (overlaps(layout[leftIndex], layout[rightIndex])) {
        throw layoutError('REPORT_LAYOUT_OVERLAP', `layout items overlap: ${layout[leftIndex].i}, ${layout[rightIndex].i}`, {
...
function packReportLayout(items, options = {}) {
  ...
  if (!Array.isArray(items)) {
    throw layoutError('REPORT_LAYOUT_INVALID', 'items must be an array');
  }
