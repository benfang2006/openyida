  ...
  const mismatches = Object.entries(expected)
    .filter(([key, value]) => {
      if (key === 'openPageAuthConfig') {
        ...
        const actual = parseOpenAuthConfig(after[key]);
        return !actual
          || JSON.stringify(canonicalize(actual)) !== JSON.stringify(canonicalize(value));
      }
...
async function run(args) {
  ...
  if (needsPreservableAuth && !parseOpenAuthConfig(before.openPageAuthConfig)) {
    throw new CliError(t('save_share_config.current_state_incomplete'), {
      code: 'SAVE_SHARE_CONFIG_CURRENT_STATE_INCOMPLETE',
      details: { before },
    });
