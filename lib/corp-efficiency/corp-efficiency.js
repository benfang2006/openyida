...
function apiGet(authRef, requestPath, params = {}, action = '请求') {
  return createYidaClient({ authRef }).getContent(requestPath, params, {
    action,
    ...API_MESSAGES,
    failMessage: `${action}失败`,
  });
}

function apiPost(authRef, requestPath, params = {}, action = '请求') {
  return createYidaClient({ authRef }).postFormContent(requestPath, params, {
    action,
    ...API_MESSAGES,
    failMessage: `${action}失败`,
  });
...
function buildReportApiInfo(report = {}) {
  ...
  return {
    ...
    notes: [
      ...
      '请求时由 OpenYida 请求工具自动携带 Bearer token。',
    ],
  };
}
