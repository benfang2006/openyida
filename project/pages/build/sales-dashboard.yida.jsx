...
export function didMount() {
  var self = this;
  this.utils.loadScript(ECHARTS_CDN).then(() => {
...
export function buildMockData(range, channel) {
  ...
  var seed = (range === '7d' ? 1.1 : range === '90d' ? 0.9 : 1.0) * (channel === 'all' ? 1 : channel === 'online' ? 0.55 : channel === 'offline' ? 0.3 : 0.15);
  var labels = [];
  var sales = [];
  var orders = [];
  var now = Date.now();
  ...
  var channels = [{
    name: '线上商城',
    value: Math.round(totalSales * 0.46),
    color: '#5C72FF'
  }, {
    name: '线下门店',
    value: Math.round(totalSales * 0.27),
    color: '#22d3ee'
  }, {
    name: '经销渠道',
    value: Math.round(totalSales * 0.18),
    color: '#a855f7'
  }, {
    name: 'KA 大客户',
    value: Math.round(totalSales * 0.09),
    color: '#f59e0b'
  }];
...
export function renderTrendChart() {
  ...
  if (!inst) {
    inst = window.echarts.init(el, null, {
      renderer: 'canvas'
    });
    ...
    series: [{
      ...
      areaStyle: {
        color: new window.echarts.graphic.LinearGradient(0, 0, 0, 1, [{
          offset: 0,
          color: 'rgba(92, 114, 255, 0.45)'
        }, {
          offset: 1,
          color: 'rgba(92, 114, 255, 0.0)'
        }])
      }
    }, {
      name: '订单量',
...
export function renderJsx() {
  ...
  return <div style={styles.page}>
      <div style={styles.header}>
        ...
        <div style={styles.toolbar}>
          <div style={styles.segment}>
            {ranges.map(r => <button key={r.key} style={styles.segmentBtn(_customState.range === r.key)} onClick={e => {
            self.changeRange(r.key);
          }}>
                {r.label}
              </button>)}
          </div>
          <div style={styles.segment}>
            {channels.map(c => <button key={c.key} style={styles.segmentBtn(_customState.channel === c.key)} onClick={e => {
            self.changeChannel(c.key);
          }}>
                {c.label}
              </button>)}
          </div>
          <button style={styles.refreshBtn} onClick={e => {
          self.refreshAll();
        }}>
            刷新
          </button>
      ...
      {loading ? <div style={styles.loadingMask}>正在加载数据...</div> : <div>
          ...
          <div style={styles.bottomRow}>
            <div style={styles.card}>
              ...
              return <div key={p.name} style={lastRow ? styles.listRowLast : styles.listRow}>
                      <div style={{
                  display: 'flex',
                  alignItems: 'center',
                  flex: 1,
                  ...
                  textAlign: 'right'
                    ...
                    color: THEME.textPrimary,
                    fontWeight: 600
                  }}>¥ {formatMoney(p.amount)}</div>
                        <div style={{
                    fontSize: '11px',
                    color: p.growth >= 0 ? THEME.good : THEME.danger,
                    marginTop: '2px'
                  }}>
            ...
            <div style={styles.card}>
              ...
              return <div key={o.id} style={lastRow ? styles.listRowLast : styles.listRow}>
                      <div style={{
                  flex: 1,
                  minWidth: 0,
                  overflow: 'hidden'
