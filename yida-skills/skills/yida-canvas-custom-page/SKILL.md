...
17. **应用级导航归平台承载**：默认不要在自定义页面中创建侧边导航、顶部应用导航、门户导航壳或同级模块菜单；同应用页面入口优先写入 `appBlueprint.navigation` 或平台导航分组，由应用导航内切换。自定义页内容区只放当前页动作、表单新建/查看、外部链接、跨应用资源。只有用户显式要求“在自定义页面中实现自己的导航 / 自绘导航 / 隐藏平台导航 / isRenderNav=false / 独立全屏门户壳”时，才执行 `use_skill("yida-nav-shell")` 并生成页面内导航壳。
