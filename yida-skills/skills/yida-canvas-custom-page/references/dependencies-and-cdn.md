...
- React、antd、Ant Design Icons、Recharts、ahooks、d3、dayjs、Radix、lucide-react、framer-motion 等包依赖必须写标准 import，例如 `import { ConfigProvider, Button } from 'antd'`。不要在源码里写 `const { ConfigProvider } = window.antd`、`const React = window.React` 或 `window.icons`；手写 window 包依赖不会进入 `importedModules`，运行时不会加载对应资源。
