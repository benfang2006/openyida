...
- `openPage` / `window.open` 更适合外部链接、新标签、钉钉链接、文件预览等场景；不要把同应用页面默认做成新窗口。
- 页面隐藏应用导航后，页面内自绘导航壳才接管跨视图切换；导航可见时不要再自绘同级应用导航。

## 设计优先级

同应用内页面优先在平台应用导航内切换。用户要求“加快捷入口/常用入口”时，先把目标页面归入应用导航或导航分组；不要在自定义页内容区重复画一排同级页面入口卡。
...
| 当前应用提交页、详情页 | `targetType: "submission"` / `"detail"`，保留目标表单和实例 ID | 同页进入平台 URL；不是新窗口，除非用户明确要求 |
| 跨应用页面 | `targetType: "page"`，保留目标 `appType` | 默认同页进入目标应用工作台；用户要求保留当前页时才新开 |
| 外部 URL、钉钉 OA、第三方系统 | `targetType: "url"` | 使用 `openPage` / 新窗口 / 钉钉打开能力 |

`linkType: "page" | "app" | "url"` 这类语义配置不要压扁成 URL 字符串。只有 `url` 类型才写普通链接。
...
Canvas 自绘快捷入口时，把路由构造收敛到一个小函数。应用内页面用同页跳转，外部链接才新开：
...
function buildYidaPath(entry, currentAppType) {
  ...
  if (entry.targetType === 'submission') return `/${appType}/submission/${entry.formUuid}`;
...
function openEntry(entry, currentAppType) {
...
- 使用可配置的门户组件时，优先保留 `linkType/page/app/url` 语义和 router 参数，不要把应用内页面写死成 `url`。
- 如果当前运行态的 `PortalQuickEntry` 只能消费静态 `url` 且点击固定 `window.open`，不要用它承载应用内页面入口；改用 Canvas 自绘入口卡片，原生组件只作为展示增强或外链入口。
