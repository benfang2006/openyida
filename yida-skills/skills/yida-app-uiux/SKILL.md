...
- 主题策略：<默认基础 token preset、themeProfile、themeScope、是否明确跟随 yida-app-theme>
...
5. **主题默认三选一**：应用默认主题先从基础样式 token preset `blue`、`green`、`orange` 三选一；用户说“宜搭应用主题风格/应用主题色”时才默认 `themeProfile.name = "yida-app-theme"`，不硬编码品牌色；用户明确要求全局导航/壳层一起换肤时再推断 `themeScope: app`。
   基础样式 token preset 用于 `style#yida-global-theme` / `customThemeStyle.tokens`，不是 `--theme` key；只有用户明确要求平台应用主题色时才使用平台 `--theme` / `colour`。
...
- 基础 token 只写 `blue` / `green` / `orange`，不写成 `podBlue` / `podGreen` / `podOrange`。
