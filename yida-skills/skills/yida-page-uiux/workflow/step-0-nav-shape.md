...
| **主色策略** | **跟随应用品牌**（`var(--color-brand1-*)`），与应用框架无缝融合，别违和 | **不必严格跟品牌**，可自立一套完整视觉体系（尤其大屏/分享/科技风）；品牌色仍是安全默认，但允许换主色相。**语义色（成功/警告/错误）永远固定**，去 AI 味红线照样生效 |
| **导航责任** | 无需自建，应用导航负责跨页跳转 | **必须自带导航壳**：应用导航没了，页面要自己承担应用级导航 |

## 导航隐藏时的自带导航壳要点

- **壳型选择**：顶部标签栏 / 左侧边栏 / 顶部条 + 面包屑——按信息广度选，别只堆一个孤零零的返回按钮。
- **多视图切换**：单个自定义页内用本地状态（native `_customState.activeView` / Canvas `useState`）或 URL hash 切换多个视图区块，做出「一个页面即一个小应用」的多样化导航，而不是一屏到底。
