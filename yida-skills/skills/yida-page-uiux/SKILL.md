...
description: >
  ...
  当用户要新建或改造自定义页面，并明确关注好看、高级、品牌化、去 AI 味、UI 视觉、美化、页面太丑、不够惊艳，或进入 yida-app fast_build 主页面实现 / 深度视觉设计阶段时触发。
...
`yida-app fast_build` 进入主页面实现前，默认把本技能作为 `ui_skill` 轻量加载。目标不是做长 PRD 或深度应用蓝图，而是在不追问的前提下给页面实现阶段一个可执行的「不要照搬 sample」护栏：
