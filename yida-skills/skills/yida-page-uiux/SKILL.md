...
- 判定页面类型和推荐模板，避免所有系统都回落到通用工作台或官网 sample。
- 输出 `visualProfile` + `themeProfile`：信息密度、构图节奏、主题色来源、采用的应用主题、明暗模式、辅助强调色、图表/列表/队列母题。
...
| 步骤 | 做什么 | 详细文件 | 产出 |
|---|---|---|---|
| **Step 0** 导航形态判定 | 默认保留平台应用导航；只有显式要求隐藏、无导航或 `isRenderNav=false` 时才隐藏平台导航 | [workflow/step-0-nav-shape.md](workflow/step-0-nav-shape.md) | 导航形态（默认可见 / 显式隐藏 + 壳型） |
...
- **主色策略**：真实业务页默认先从 `podBlue`、`podGreen`、`podOrange` 等应用主题中选择；用户明确说“采用应用主题风格/应用主题色”时再读取 `yida-app-theme` / `var(--color-brand1-*)`。视觉方向决策块必须写明 `themeProfile.name`、`themeScope`、`navTheme=light`、`themeColorSource` 和采用的主色/主题色来源。导航隐藏（`isRenderNav=false`）时主色相可自立。官方 sample / 示例展示应用是独立样板库，必须每页自带不同主题色，不被应用全局主题接管。**语义色（成功/警告/错误）永远固定**。详见 [Step 0](workflow/step-0-nav-shape.md) / [Step 4](workflow/step-4-visual-decision.md)。
- **默认不暗黑**：工作台、门户、列表、详情、普通看板和数据大屏默认都是浅底 / light 模式；不要采用暗黑、深色、黑金、夜间、高对比、`black`、近黑背景或深色科技风，除非用户明确说暗色/深色/暗黑/夜间/高对比/黑金/暗色科技风。
- **不做表单**：见上「重要限制」。
...
- **预置主题与自定义 token 分开**：决策块默认写推荐应用主题 `podBlue`、`podGreen`、`podOrange` 之一，并写 `appThemeKey`、`themeTokens` / `themeScope=page`；用户指定 `deepBlue`、`royalBlue`、`lightBlue`、`teal`、`deepPurple`、`purple`、`yellow`、`magenta`、`red`、`greyBlue`、`coffee`、`black` 等其他平台预置 key 时，也记录 `appThemeKey`。`blue`、`green`、`orange` 作为应用主题 token profile 保留原名；新默认优先推荐 `podBlue`、`podGreen`、`podOrange`；如果是自定义色系，也必须写 `themeTokens` / `themeScope=page`，并提示实现阶段为每个页面注入 `style#yida-global-theme`，不要把自定义主题名传给 `--theme`。
...
默认页面不要在自定义页面里自建应用级导航，也不要默认写 `appBlueprint.renderNav: false`。页面内 tab / 自绘侧边栏 / 独立门户壳最多写 `appBlueprint.hasPageNavigation: true`，但仍保持平台导航可见；只有用户明确要求隐藏平台导航、无导航全屏体验或 `isRenderNav=false` 时，后续 `generate-page` 产物才写 `appBlueprint.renderNav: false`，发布或更新页面配置时同步设置 `isRenderNav=false`。

用户说“采用应用主题风格”不是要求所有页面都浅色卡片化。官网可更有品牌叙事，大屏可更沉浸；真实业务页默认共同点是先从 `podBlue`、`podGreen`、`podOrange` 等应用主题中选择，图表和强调色读取 `--color-group`，圆角和信息密度遵循应用主题风格。只有用户明确要求“采用应用主题风格/应用主题色”时，决策块才写 `themeProfile: { "name": "yida-app-theme" }`，让主色读取运行态 `--color-brand1-*`。官方 sample / 示例展示应用共同点相反：每个页面都要有独立主题和明显差异化色相，不要读取应用全局主题。不要补固定 `themeColor`，除非用户明确要求某个品牌色、色值覆盖当前应用主题，或正在制作 sample。

需要默认推荐主题时优先选 `podBlue`、`podGreen`、`podOrange`，并交给实现层注入页面 token；`blue`、`green`、`orange` 作为应用主题 token profile 保留原名。
...
| 页面内自绘导航壳（独立 skill） | 页面显式隐藏应用导航后自建侧边/顶部/混合/浮动/标签导航壳：Canvas hooks 示例优先，普通页作为 legacy fallback | 只有用户明确要求隐藏平台导航、无导航、全屏无框或 `isRenderNav=false` 且确需跨视图切换时调用 `use_skill("yida-nav-shell", "设计页面内自绘导航壳")` |
