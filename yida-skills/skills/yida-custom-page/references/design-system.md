...
> **本文件是平台 JSX 组件页面的 token / 组件实现适配层（数值层）**：提供色彩、圆角、字体、间距的具体值与组件模板，不产出配色、视觉 DNA 或页面风格。业务事实来自 `yida-prd` 输出的 `prd.md`，视觉事实来自 `yida-design` 输出的 `design.md`；本文件只负责把 `design.md` 的视觉方向落到平台 JSX 组件页面的内联样式、Tailwind className、组件和状态上。
