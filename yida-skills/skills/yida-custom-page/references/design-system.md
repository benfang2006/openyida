...
> **本文件是 token / 组件实现层（数值层）**：提供色彩/圆角/字体/间距的具体值与组件模板。视觉**方向与差异化**（选哪种气质、如何偏离默认脸、去 AI 味）由 `yida-design` 子技能决定——需要时调用 `use_skill("yida-design", "确定自定义页面视觉方向")`，**先定方向、再用本文件落地**。直接套用本文件的默认组合（灰白底 + 统一圆角卡片）而不先做方向决策，正是页面「有 AI 味」的根因。
...
实现前先用这一组规则统一检查页面设计；视觉方向由 `yida-design` 决定，本文件只负责把方向落到 token、组件和状态上。
