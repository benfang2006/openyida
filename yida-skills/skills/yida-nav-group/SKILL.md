...
- 完整应用首次生成后，必须基于业务信息架构决定根导航顺序，不能保留创建时的默认顺序（默认常是表单在前、看板在后）。`fast_build` 发布主页面成功后默认立刻执行一次轻量自动排序。
- **默认自动排序优先级**：门户/首页/工作台入口 > 自定义展示页面 > 流程表单 > 普通表单。没有更细业务旅程时，直接用 `openyida nav-group auto-order <appType>`。
...
`auto-order` 会读取当前根导航并按默认优先级排序：门户/首页/工作台入口 > 自定义展示页面 > 流程表单 > 普通表单；系统导航保持在系统区域，未识别分组和其他项排在后面。适合 `fast_build` 发布主页面后立刻跑一次，避免长上下文漏掉手写 `order` 参数。
