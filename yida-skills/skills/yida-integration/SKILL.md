...
| `--get-self-query-field <field>` | 自动按来源表单类型选择 | 覆盖左侧查询系统字段；流程表单使用 `pid` / `流程实例ID`，普通表单使用 `form_inst_id` / `表单实例ID`；仅在确认平台字段名不同后使用 |
...
- 获取自身：优先使用 `--get-self`，流程表单标准条件为查询侧 `pid（流程实例ID）` 等于触发事件字段 `__masterdata_form_inst_id`，普通表单使用 `form_inst_id（表单实例ID）`。不要用 `formInstId = formInstId`、不要用“包含”或非唯一字段做自身匹配。
