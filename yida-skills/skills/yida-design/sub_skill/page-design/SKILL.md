...
| `project/config.json`、`.cache/<项目名>-schema.json`、`.openyida-page.json` | 已记录的 app/page/form、themeProfile、themeScope、visualProfile | `themeEvidence.source=workspace` |
...
主题证据齐全时，页面重构、局部美化、列表/看板/详情优化默认沿用当前应用 `colour` / `themeProfile.appThemeKey`。主题证据缺失时，记录 `themeEvidence.status=missing`，并从 `podBlue`、`podGreen`、`podOrange` 中选择最贴合业务的页面级主题。用户明确要求完全不同的品牌风格、独立活动/官网页、隐藏导航沉浸页或给出新品牌色时，再写 `themeDecision=page-independent`。
...
| 2 | [需求定位](../../workflow/step-1-positioning.md) | 聚焦当前页面的用户、任务、业务对象和 UI-only 改造目标 |
| 3 | [主题系统](../../workflow/step-2-theme-system.md) | 基于 Step 1 的应用主题，优先跟随应用主题；强差异化诉求再决定 `themeScope=page` 独立覆盖 |
| 4 | [信息架构](../../workflow/step-3-information-architecture.md) | 只补当前页与平台导航、上游入口、下钻页面、原生表单/流程的关系 |
| 5 | [页面原型与交互](../../workflow/step-4-wireframe-interaction.md) | 明确布局骨架、主操作、详情抽屉、表单提交入口和 PC/移动端差异 |
| 6 | [高保真视觉与状态](../../workflow/step-5-visual-states.md) | 细化当前页视觉、素材、图标、空态、加载态、错误态和业务化自检 |
| 7 | [交付 PRD](../../workflow/step-6-handoff.md) | 输出单页 PRD 章节，交给 Code Canvas 或普通自定义页实现 |
...
- 用户明确要全应用换肤：将诉求回到 `yida-design` 主题系统分支，输出 `themeScope=app` 和 `customThemeStyle.tokens`。
- 单页只做局部美化：保持平台导航和应用主题稳定，页面级 `style#yida-global-theme` 只覆盖当前页视觉变量。

## 输出补充字段

在 [PRD 输出格式](../../workflow/output-prd.md) 基础上补充：
