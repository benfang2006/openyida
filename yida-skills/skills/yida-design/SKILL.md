...
9. **主题统一在应用级交付**：所有页面都使用同一应用主题。新版应用一律基于 `references/theme/app-custom-theme-template.css` 生成并上传自定义主题文件；文件完整保留 `--color-brand1-1` 至 `--color-brand1-10`，主色写入 `--color-brand1-6`，CLI 校验完整色阶并自动以主色保存 `themeColor`。单页美化沿用该主题，只调整布局、材质、密度、素材、构图和辅助视觉。
