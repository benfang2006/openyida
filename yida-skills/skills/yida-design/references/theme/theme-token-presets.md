...
注意：下方预置只作为 AI 设计色阶时的参考，不再限制新版应用主题。历史 profile 只记录平台已有档位；复制 [app-custom-theme-template.css](app-custom-theme-template.css) 生成应用主题文件时，必须补齐并完整保留 `--color-brand1-1` 至 `--color-brand1-10`，将任意合法主色写入 `--color-brand1-6`，再通过 `create-app/update-app --theme-file/--nav-theme/--logo-source/--layout` 联合保存。主题由服务端加载该文件，页面和表单只消费其中的变量。
...
每个 profile 都按平台变量名记录。profile 未列出的 `--color-brand1-4`、`--color-brand1-5`、`--color-brand1-7`、`--color-brand1-8` 由 AI 根据主色推导，并写入最终主题文件。`--color-brand1-*` 是页面和 PC 端主要消费的品牌色阶，`--color-brand-*` 是移动端和部分原生表单/壳层桥接仍会消费的品牌色阶，不能删掉、改名或替换为别的变量。`--color-group` 用于图表和分组配色。
