...
- 页面实现阶段读取 `prd.md` 的业务内容，并直接读取 `design.md` 的主题、布局、材质、圆角、密度、呼吸感、组件和状态规则；只有走页面生成器或需要稳定交接时才派生 `page-spec.json`，再交给 `yida-canvas-custom-page` 或 `yida-custom-page` 实现。已检测到平台 JSX 组件页面维护时才使用 `yida-custom-page`。
