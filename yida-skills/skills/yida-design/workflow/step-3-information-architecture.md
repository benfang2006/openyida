...
| 概念 | 控制位置 | 典型信号 | 设计输出 |
| --- | --- | --- | --- |
| 应用导航隐藏 | 应用基础设置 `hideAppNav='y'` | 自定义页要做顶部导航、侧边导航、导航壳、自绘应用级导航 | 写 `appBlueprint.hideAppNav: 'y'`，并交给 `yida-nav-shell` |
| 页面导航隐藏 | 页面配置 `isRenderNav=false` | 页面隐藏导航、无导航、全屏无框、独立分享页 | 写页面级隐藏分支，不自动写 `appBlueprint.hideAppNav` |

默认规则：

- 默认保留平台应用导航。
- 默认自定义页**保留平台应用导航**。
- 页面内 tab / 分段导航 / 内容区局部导航记录为当前页内容结构，同时保持平台导航可见。
...
- 不要用 `isRenderNav=false` 代替应用基础设置里的 `hideAppNav`。
- 不要让宜搭平台应用导航和自绘应用级导航同时出现。
