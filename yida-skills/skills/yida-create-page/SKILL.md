...
- 如果本轮用户给了页面 URL、`formUuid`、bound page，或 workspace cache/config 中已有目标 display page，禁止调用 `openyida create-page`；改由 `yida-canvas-custom-page` 或 `yida-custom-page` 编写页面源码，再由 `yida-publish-page` 发布。
...
- 创建成功后，将真实 `formUuid` 交给 `yida-canvas-custom-page` 编写页面源码，再交给 `yida-publish-page` 发布；已检测到平台 JSX 组件页面维护时，源码开发交给 `yida-custom-page`。
