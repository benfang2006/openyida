...
description: 宜搭普通自定义页面（native）设计器 dataSourceMap 专用技能。仅用于把连接器/远程 API 注册到 Page dataSource.online 并通过 this.dataSourceMap.<name>.load() 调用。`YidaCodeCanvas` 组件没有 dataSourceMap；使用 `YidaCodeCanvas` 组件实现的自定义页面数据接入必须转 yida-canvas-data-binding / HTTP 数据桥。
---
...
它不是 `YidaCodeCanvas` 组件页面通用数据接入技能。`YidaCodeCanvas` 组件没有普通页面 `this` 实例，也没有 `dataSourceMap`；遇到 `.canvas.jsx`、`YidaCodeCanvas`、`YidaComp`、React hooks 或现代自定义页面看板时，立即转：
...
不要为了使用本技能而把 `YidaCodeCanvas` 组件页面降级成普通自定义页面。
...
详细规则由 `yida-canvas-data-binding` 决定。本技能只负责判断“当前需求不是 native dataSourceMap”并完成转交，不为 `YidaCodeCanvas` 组件页面发明伪 `dataSourceMap`。
...
这些是普通自定义页面验证步骤。使用 `YidaCodeCanvas` 组件实现的页面验证必须使用 `compileCanvasLocal`、`.canvas.jsx` 发布和 `YidaCodeCanvas/runtimeCode` 回读，不要复用这里的 `check-page` 默认。
...
- 已确认目标是普通自定义页面，而不是 `YidaCodeCanvas` 组件页面。
...
- 如果目标是 `YidaCodeCanvas` 组件页面，已转 `yida-canvas-data-binding`，本技能没有生成 native 代码。
