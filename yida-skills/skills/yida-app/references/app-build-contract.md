...
- 截图验收、公开访问、数据源深接和精细导航分组只在用户明确要求或 PRD 验收标准命中时追加。完整应用默认在表单创建后写入 1-3 条核心普通表单 seed records，不再作为可选后置。
...
8. 输出一个主访问链接和业务交付总结，例如“创建访客登记表、访问记录表 2 个表单；创建访客工作台 1 个页面；完成访客登记、访问状态跟踪和最近访问查看”。
...
完整应用 final 只保留一个主访问链接，再用业务语言总结交付内容。不要默认列资源 ID 表格。

推荐写法：

- 创建了哪些表单：例如订单表单、客户表单、商品表单等 5 个表单。
- 创建了哪些页面：例如首页、订单管理、库存看板、数据看板等 4 个页面。
- 完成了哪些功能：例如订单录入、库存预警、销售统计、表单提交入口、详情查看、导航排序。
- 示例记录：说明已写入 1-3 条核心表单记录，或说明跳过原因。
