---
name: yida-app
description: 宜搭完整应用开发技能。从零到一搭建完整宜搭应用的全流程，包括创建应用、创建页面、需求分析、编写代码、创建表单、发布部署。适用于全新应用搭建场景。
---
...
为避免生成错误代码，优先用 `openyida sample` 获取 CLI 内置模板，不要读取不存在的 `skills/*/templates` 路径：

| 技能 | 模板获取命令 | 用途 |
|------|---------|------|
| yida-custom-page | `openyida sample yida-custom-page custom-page-template` | 自定义页面完整模板 |
...
1. 执行对应的 `openyida sample` 命令生成模板
2. 以模板为基础进行扩展
3. 验证所有参数名称与 CLI 一致
...
（描述页面的核心功能、交互逻辑、业务规则、状态流转、异常处理）

## 页面与表单配置

### 用户信息表（表单页面）

| 字段名称 | 宜搭字段类型 | 必填 | 选项/格式 | 说明 |
|---------|--------------|------|-----------|------|
| 姓名 | TextField | 是 | - |  |
| 部门 | SelectField | 是 | 技术部、产品部、运营部 |  |
| 备注 | TextareaField | 否 | - |  |
...
- PRD 只记录业务语义信息，不记录 `formUuid`、`fieldId` 等 Schema ID
- Schema ID 统一写入 `.cache/<项目名>-schema.json`
- 用户主入口优先使用自定义页面；后台表单仅作为数据维护入口，除非需求明确要求原生表单
- 自定义页面默认使用 Tailwind 风格层并开启 preflight 重置原生控件外观；用户可见下拉框禁止原生 `<select>`，使用自定义 dropdown 交互
- 自定义页面发布前必须执行 `openyida check-page` 和 `openyida compile`
...
> - PRD 必须说明 MVP 边界、角色权限、核心用户旅程、状态机、数据约束和验收标准；不明确的信息写入「待确认事项」
...
**代码文件路径**：`pages/src/<项目名>.oyd.jsx`

**核心规范**：
- 使用 `export function` 导出函数（`didMount`、`didUnmount`、`renderJsx` 三个必须导出）
- `renderJsx` 顶部定义 `var self = this`，事件绑定统一写 `(e) => { self.xxx(e); }`，禁止直接 `onClick={this.xxx}` 或 `.bind(this)`
- `renderJsx` 每个 return 分支都必须有隐藏 timestamp 节点；`.oyd.jsx` 构建会自动补齐，但模板里仍建议显式保留
- 工作台/看板默认要有空态或演示数据兜底，接口失败时把 `loading` 置回 `false`，避免首屏永久显示加载文案
- 状态管理使用 `_customState` + `setCustomState`
- 输入框使用非受控组件（`defaultValue` 而非 `value`）
- 动态字段对象禁止写 `{ [key]: value }` 或 `setCustomState({ [key]: value })`；先 `var obj = {}; obj[key] = value;`，否则 `check-page` 会以 `computed-property` error 阻塞
- 默认使用 Tailwind utility `className` 组织视觉层，开启 preflight，并保留关键 `style` 兜底；不要写原生 `<select>` 下拉
- 列表/表格类页面：参考 `yida-density` 技能选择合适的信息密度

**预检命令**：

```bash
openyida check-page pages/src/<项目名>.oyd.jsx
...
| Babel 编译失败 | 检查 JSX 语法；现代 React authoring 源码应使用 `.oyd.jsx` 并通过 OpenYida 兼容编译，参考 `yida-custom-page` 规范 |
