...
表单布局组件应跟随运行态主题变量。本参考只说明字段 JSON 如何消费主题；OpenYida 在表单创建和保存时默认把 `style#yida-global-theme` 注入代码写入表单 JS。
...
- 表单 JS 默认注入 `style#yida-global-theme`，并尽可能同步到当前页面和同源 `window.top`；跨域窗口只跳过，不报错。
...
执行规则：OpenYida 必须在表单 JS 中默认注入运行时主题样式 `style#yida-global-theme`，并尽可能同步到当前页面和同源 `window.top`。
