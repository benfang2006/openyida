...
OpenYida 任务完成后，除了交付当前结果，还要判断是否有可复用经验需要落盘到 CLI、sample、测试或 skill。目标是让下一次类似任务更少踩坑，而不是只把本次问题修好。
...
- 新增或改造了 `lib/samples/**` 模板，且经验会影响同类 sample 的质量。
...
| 技能路由、工作流、验收纪律 | `yida-skills/SKILL.md` 或对应子技能 `SKILL.md` | 主题任务必须先读 `yida-theme`，页面视觉先读 `yida-page-uiux` |
| 页面视觉、Dribbble/优秀案例参考方法 | `yida-page-uiux` workflow/reference | 参考案例要转译为布局、视觉锚点、密度、色彩、组件细节 |
| Code Canvas / sample 共性规则 | `yida-canvas-custom-page` | sample 必须页面级独立主题、丰富模拟数据、发布后回读 Schema |
| 应用主题、全局 CSS 变量 | `yida-theme` | `--theme` 只接受平台预置 key，自定义主题走 `style#yida-global-theme` |
| 表单/流程/报表等领域规则 | 对应子技能 | 字段、公式、流程规则、报表配置分别沉淀到所属 skill |
| 临时经验但暂不适合入主技能 | 子技能 references 或新增 reference | 一组可复用接口探测、截图验收清单、素材流程 |

## 本轮官方 sample 改造沉淀

这轮官方 Samples 展示应用的批量改造暴露出的经验应长期保留。以后用户继续要求“优化 sample / 同步到前面改进的应用 / 看起来更高级 / 去 Dribbble 参考 / 加真实数据”时，按下面规则执行。
...
6. **截图反馈要归因并反哺**
   用户基于截图指出问题时，先判断是主题污染、素材缺失、布局断层、数据不足、组件 props 缺失还是模板缺口。属于共性问题就补 skill、模板或测试。

7. **工作台是操作首页，不是 demo 页面**
...
| `workbench-home` / 工作台 | 深玫红，不能有留白，是真工作台 | 全屏操作面板，侧栏贴边、主区密集，包含待办、行动队列、指标、快捷入口、动态流；去掉设计过程标签。 |
...
| native 系列全部 sample | 都要去 Dribbble 搜同类优秀设计 | 先确定“组件实验室 / 业务门户 / 数据管理视图”的产品形态，再接运行态组件；失败只降级局部区域。 |

### 官方 sample 发布验收链路

1. 查看 `.cache/openyida-official-samples-gallery-schema.json`，确认 sample 对应 `formUuid`。
2. 修改 `lib/samples/**` 后先跑 `compileCanvasLocal` 或对应编译链路。
3. 跑相关 Jest，例如 `tests/canvas-compile.test.js`、`tests/generate-page.test.js`，并补防回归断言。
4. 跑 `git diff --check`，避免空白错误。
5. 发布到官方 Samples 应用后，用 `get-schema` 回读确认 `YidaCodeCanvas/runtimeCode` 更新，并 grep 关键 class / 文案 / 样式。
6. 如果发现“接口成功但线上没变”，优先修 CLI / 测试 / skill，不要只手工绕过。

### Chart sample / 原生报表绑定经验
...
1. **能用测试防回归**：补 `tests/**`。
2. **属于页面模板共性**：补 `lib/samples/**` 或 `generate-page` 模板。
...
4. 是否新增/修改 sample，需要跑相关 Canvas 编译、Jest 和 `git diff --check`。
